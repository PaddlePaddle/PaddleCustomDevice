...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ReluEquivalenceTrans) {
...
// dx = dy if y > 0 else 0
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ReluGradEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, Relu6EquivalenceTrans) {
  auto in = map_inputs["X"].at(0);
  auto rank = in->GetType().GetShape().size();
  if (running_mode == RunningMode::ADAPTIVE) {
    if (rank == 4) {
...
//           \ 0, x >= 6
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, Relu6GradEquivalenceTrans) {
  builder::Op out = *(map_inputs["Out"].at(0));
  builder::Op ddx = *(map_inputs["Out@GRAD"].at(0));
  auto rank = out.GetType().GetShape().size();
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SigmoidEquivalenceTrans) {
  return std::make_shared<GcuOp>(builder::Sigmoid(*(map_inputs["X"].at(0))));
}

IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SigmoidGradEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, HardSwishEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, HardSwishGradEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, HardSigmoidEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SiluEquivalenceTrans) {
  builder::Op x_op = *(map_inputs["X"].at(0));
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SiluGradEquivalenceTrans) {
  builder::Op x_op = *(map_inputs["X"].at(0));
  builder::Op dout_op = *(map_inputs["Out@GRAD"].at(0));
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, PowEquivalenceTrans) {
  ...
  if (map_inputs.count("FactorTensor") != 0 &&
    ...
    if ((factor_shape[0] != 1) || (factor_shape.size() != 1)) {
      PADDLE_THROW(
          phi::errors::InvalidArgument("Factor_shape(axis)"
                                       "can only be [1]"));
    }
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, PowGradEquivalenceTrans) {
  builder::Op x_op = *(map_inputs["X"].at(0));
  builder::Op dout_op = *(map_inputs["Out@GRAD"].at(0));
  ...
  if (map_inputs.count("FactorTensor") != 0 &&
    ...
    if ((factor_shape[0] != 1) || (factor_shape.size() != 1)) {
      PADDLE_THROW(
          phi::errors::InvalidArgument("Factor_shape(axis) can only be [1]"));
    }
    if (factor_op.GetType().GetPrimitiveType() != ptype)
      factor_op = builder::Convert(factor_op, builder::Type({1}, ptype));
  } else {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SwishEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SwishGradEquivalenceTrans) {
  builder::Op x_op = *(map_inputs["X"].at(0));
  builder::Op dout_op = *(map_inputs["Out@GRAD"].at(0));
  ...
  float beta = PADDLE_GET_CONST(float, op->GetAttr("beta"));
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, LeakyReluEquivalenceTrans) {
  auto x = *(map_inputs["X"].at(0));
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, LeakyReluGradEquivalenceTrans) {
  builder::Op x_op = *(map_inputs["X"].at(0));
  builder::Op dout_op = *(map_inputs["Out@GRAD"].at(0));
  auto rank = x_op.GetType().GetShape().size();
  if (running_mode == RunningMode::ADAPTIVE) {
  ...
  float alpha = PADDLE_GET_CONST(float, op->GetAttr("alpha"));
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SquareEquivalenceTrans) {
  auto x = *(map_inputs["X"].at(0));
  return std::make_shared<GcuOp>(builder::Square(x));
}

IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SquareGradEquivalenceTrans) {
  auto x = *(map_inputs["X"].at(0));
