...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
  ...
  if (!test_mode) {
    ...
    std::vector<GcuOp> outputs{output_y,
                               ...
                               current_mean,
                               current_variance};
    // if (output_name_map.count("ReserveSpace") != 0) {
    //   std::vector<float> tmp = {0};
    //   auto place_holder = builder::Const(
    //       gcu_builder, static_cast<void *>(tmp.data()), GcuType({}, ptype));
    //   output_names.emplace_back("ReserveSpace");
    ...
    std::vector<GcuOp> outputs{output_y,
                               ...
                               current_mean,
                               current_variance};
    // add last output_y for place holder
    // if (output_name_map.count("ReserveSpace") != 0) {
    //   auto place_holder = builder::Const(gcu_builder, 0, GcuType({1},
    //   ptype)); output_names.emplace_back("ReserveSpace");
    //   outputs.emplace_back(place_holder);
    // }
    for (size_t i = 1; i < output_names.size(); ++i) {
      output_names_attr += ";" + output_name_map[output_names[i]][0];
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
  ...
  // GcuOp reserve_space = *(map_inputs["ReserveSpace"].at(0));
