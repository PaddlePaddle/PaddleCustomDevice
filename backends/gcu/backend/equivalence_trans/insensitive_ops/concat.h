...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ConcatEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ConcatGradEquivalenceTrans) {
  ...
  if (input_names.size() == output_names.size()) {
    ...
    for (const auto &name : output_names) {
      ...
      if (iter == input_names.end()) {
        PADDLE_THROW(phi::errors::Unimplemented("unvalid output name of : %s",
                                                name.c_str()));
      }
