...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, Conv2DEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, Conv2DGradEquivalenceTrans) {
  ...
  if (running_mode == RunningMode::ADAPTIVE) {
    ...
    // TransformUtil::TransAndRecordWeights(op, "Filter",
  ...
  if (output_name_map.count("Filter@GRAD") != 0 &&
    ...
    if (running_mode == RunningMode::ADAPTIVE) {
      ...
      // TransformUtil::TransAndRecordWeights(op, "Filter@GRAD",
