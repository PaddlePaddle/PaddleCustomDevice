...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, CrossEntropyEquivalenceTrans) {
  ...
  if (soft_label) {
    ...
    if (input_type != label_type) {
      PADDLE_THROW(
          phi::errors::InvalidArgument("Labels type and Logits type mismatch"));
    }
    ...
    PADDLE_THROW(
        phi::errors::Unimplemented("GCU not support, It's coming soon!!"));
  }
}
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, CrossEntropy2EquivalenceTrans) {
  builder::Op inputs = *(map_inputs["X"].at(0));
  builder::Op label_op = *(map_inputs["Label"].at(0));
  auto ignore_index = PADDLE_GET_CONST(int, op->GetAttr("ignore_index"));
