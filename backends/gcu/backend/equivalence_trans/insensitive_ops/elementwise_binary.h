...
#define BINARY_OP(name)                                                        \
  IMPLEMT_EQUIVALENCE_TRANS_FUNC(                                              \
      gcu_builder, op, map_inputs, running_mode, name##EquivalenceTrans) {     \
    std::vector<GcuOpPtr> inputs;                                              \
    if (map_inputs.count("X") != 0) {                                          \
      inputs.push_back(map_inputs["X"].at(0));                                 \
    } else {                                                                   \
      PADDLE_ENFORCE_EQ(                                                       \
          true, false, phi::errors::NotFound("lack of [X] gcu op"));           \
    }                                                                          \
    if (map_inputs.count("Y") != 0) {                                          \
      inputs.push_back(map_inputs["Y"].at(0));                                 \
    } else {                                                                   \
      PADDLE_ENFORCE_EQ(                                                       \
          true, false, phi::errors::NotFound("lack of [Y] gcu op"));           \
    }                                                                          \
                                                                               \
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ElementWisePowEquivalenceTrans) {
  std::vector<GcuOpPtr> inputs;
  if (map_inputs.count("X") != 0) {
    inputs.push_back(map_inputs["X"].at(0));
  } else {
    PADDLE_ENFORCE_EQ(true, false, phi::errors::NotFound("lack of [X] gcu op"));
  }
  if (map_inputs.count("Y") != 0) {
    inputs.push_back(map_inputs["Y"].at(0));
  } else {
    PADDLE_ENFORCE_EQ(true, false, phi::errors::NotFound("lack of [Y] gcu op"));
  }
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, AddGradEquivalenceTrans) {
...
// subgrad => x`: 1, y`: -1
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SubGradEquivalenceTrans) {
  GcuOp lhs_op = *(map_inputs["X"].at(0));
  GcuOp rhs_op = *(map_inputs["Y"].at(0));
  GcuOp out_grad_op = *(map_inputs["Out@GRAD"].at(0));
...
// mulgrad => x`: y, y`: x
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, MulGradEquivalenceTrans) {
  GcuOp lhs_op = *(map_inputs["X"].at(0));
  GcuOp rhs_op = *(map_inputs["Y"].at(0));
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, DivGradEquivalenceTrans) {
  GcuOp lhs_op = *(map_inputs["X"].at(0));
  GcuOp rhs_op = *(map_inputs["Y"].at(0));
  GcuOp out_grad_op = *(map_inputs["Out@GRAD"].at(0));
