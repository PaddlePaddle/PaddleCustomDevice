...
namespace backend {
const char *const kConvBn = "conv_bn";
const char *const kConvBnSwish = "conv_bn_swish";

IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ConvBnEquivalenceTrans) {
  using backend::EquivalenceTransformer;
  auto x = *(map_inputs["X"].at(0));
  auto filter = *(map_inputs["Filter"].at(0));

  VLOG(10) << "=== Start ConvBnRelu trans ===";
  // attrs
...
EQUIVALENCE_TRANS_FUNC_REG(kConvBn, INSENSITIVE, ConvBnEquivalenceTrans);

IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ConvBnSwishEquivalenceTrans) {
  using backend::EquivalenceTransformer;
  auto x = *(map_inputs["X"].at(0));
  auto filter = *(map_inputs["Filter"].at(0));
  ...
  if (in_rank == 4) {
    return std::make_shared<GcuOp>(builder::Transpose(
        builder::Swish(builder::Transpose(output_y, {0, 2, 3, 1})),
        {0, 3, 1, 2}));
  } else if (in_rank == 5) {
    return std::make_shared<GcuOp>(builder::Transpose(
        builder::Swish(builder::Transpose(output_y, {0, 2, 3, 4, 1})),
        {0, 4, 1, 2, 3}));
  }
  return std::make_shared<GcuOp>(builder::Swish(output_y));
}

EQUIVALENCE_TRANS_FUNC_REG(kConvBnSwish,
                           INSENSITIVE,
                           ConvBnSwishEquivalenceTrans);
}  // namespace backend
