...
namespace backend {
const char *const kConvBnRelu = "conv_bn_relu";

IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, ConvBnReluEquivalenceTrans) {
  ...
  if (in_rank == 4) {
    return std::make_shared<GcuOp>(builder::Transpose(
        builder::Relu(builder::Transpose(output_y, {0, 2, 3, 1})),
        {0, 3, 1, 2}));
  } else if (in_rank == 5) {
    return std::make_shared<GcuOp>(builder::Transpose(
        builder::Relu(builder::Transpose(output_y, {0, 2, 3, 4, 1})),
        {0, 4, 1, 2, 3}));
  }
  return std::make_shared<GcuOp>(builder::Relu(output_y));
}

EQUIVALENCE_TRANS_FUNC_REG(kConvBnRelu,
                           INSENSITIVE,
                           ConvBnReluEquivalenceTrans);
