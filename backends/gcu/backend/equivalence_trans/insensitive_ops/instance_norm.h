...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, InstanceNormEquivalenceTrans) {
  ...
  if (rank <= 2 || rank > 5) {
    PADDLE_THROW(
        phi::errors::InvalidArgument(" expected 3D or 4D or 5D input."));
  }
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(gcu_builder,
  ...
  PADDLE_ENFORCE_EQ(N * C, NxC, phi::errors::InvalidArgument("interanl error"));
