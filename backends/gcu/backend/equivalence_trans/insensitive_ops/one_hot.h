...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, OneHotEquivalenceTrans) {
  ...
  if (static_cast<phi::DataType>(out_dtype) == phi::DataType::FLOAT32) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, OneHotV2EquivalenceTrans) {
  ...
  if (static_cast<phi::DataType>(dtype) == phi::DataType::FLOAT32) {
    ...
    PADDLE_THROW(phi::errors::Unimplemented("one_hot_v2 dtype: %d", dtype));
  }
