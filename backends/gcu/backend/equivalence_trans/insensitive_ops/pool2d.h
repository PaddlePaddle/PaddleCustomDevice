...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, Pool2DEquivalenceTrans) {
  auto adaptive = false;
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, Pool2DGradEquivalenceTrans) {
  ...
  if (pooling_type == "max") {
    if (global_pooling) {
      PADDLE_THROW(
          phi::errors::Unimplemented("Unsupported global "
                                     "pooling_type: %s",
                                     pooling_type.c_str()));
      return nullptr;
    }
    ...
    PADDLE_THROW(
        phi::errors::Unimplemented("Unsupported pooling_type:"
                                   "%s",
                                   pooling_type.c_str()));
    return nullptr;
  }
