...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SliceEquivalenceTrans) {
  ...
  auto GetShapeByPerm = [](const std::vector<int64_t> &shape,
    ...
    PADDLE_ENFORCE_EQ(
        shape.size(),
        perm.size(),
        phi::errors::Fatal("When Slice op do trans shape, in shape "
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SliceGradEquivalenceTrans) {
  ...
  builder::Op padding_value_op;
  {
    ...
    if (input_dtype == builder::PrimitiveType::F32()) {
      ...
      PADDLE_THROW(
          phi::errors::Unimplemented("GCU slice_grad NOT support data type."));
    }
  }
