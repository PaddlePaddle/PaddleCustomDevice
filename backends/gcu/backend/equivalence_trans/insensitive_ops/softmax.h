...
#define HLIR_CONST(T)                                                    \
  ...
  if (primitive_type == builder::PrimitiveType::S8()) {
    ...
    PADDLE_THROW(phi::errors::Unimplemented("Unable to lower scalar %d of type",
                                            primitive_type));
  }
#undef HLIR_CONST
  return hop;
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SoftmaxEquivalenceTrans) {
  ...
  if (input.GetType().GetPrimitiveType() == builder::PrimitiveType::F32()) {
    ...
    PADDLE_THROW(phi::errors::InvalidArgument("UNsupport datatype"));
  }
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, SoftmaxGradEquivalenceTrans) {
  int64_t axis =
