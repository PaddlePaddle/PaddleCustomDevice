...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, unsqueezeEquivalenceTrans) {
  ...
  if (map_inputs.count("AxesTensor") != 0) {
    ...
    PADDLE_THROW(
        phi::errors::Unimplemented("Unsupported unsqueeze without axes"));
  }
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, unsqueezeGradEquivalenceTrans) {
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, unsqueeze2GradEquivalenceTrans) {
  GcuOp out_grad = *(map_inputs["Out@GRAD"].at(0));
