...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, UnstackEquivalenceTrans) {
  ...
  if (dim_at_axis > 0 && dim_at_axis != num) {
    PADDLE_THROW(phi::errors::InvalidArgument("Expect num == x_dim[axis]"));
  }
...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, UnstackGradEquivalenceTrans) {
