...
IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, WhereEquivalenceTrans) {
  GcuOp x;
  GcuOp y;
  ...
  if (x.IsValid() && y.IsValid()) {
    ...
    PADDLE_THROW(phi::errors::InvalidArgument("X and Y must Be inputed!"));
  }
  return std::make_shared<GcuOp>(result);
}

IMPLEMT_EQUIVALENCE_TRANS_FUNC(
    gcu_builder, op, map_inputs, running_mode, WhereGradEquivalenceTrans) {
  ...
  if (output_name_map.count("X@GRAD") != 0 &&
    ...
    PADDLE_THROW(
        phi::errors::InvalidArgument("grad_x or grad_y must be output!"));
