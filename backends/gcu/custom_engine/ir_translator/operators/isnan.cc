...
static GcuOpPtr TranslateIsnan(
    GcuBuilderPtr gcu_builder,
    const pir::Operation *op,
    const std::vector<std::vector<GcuOpPtr>> &gcu_op_inputs) {
  auto input = *(gcu_op_inputs[0][0]);
  float nan_data = std::numeric_limits<float>::quiet_NaN();
  auto nan_op = builder::FullLike(input, nan_data);
  auto abs_in = builder::Abs(input);
  auto result = builder::Compare(abs_in, nan_op, "EQ");
  return std::make_shared<GcuOp>(result);
}

}  // namespace custom_engine

REGISTER_OP_TRANSLATOR(pd_op_isnan, custom_engine::TranslateIsnan)
