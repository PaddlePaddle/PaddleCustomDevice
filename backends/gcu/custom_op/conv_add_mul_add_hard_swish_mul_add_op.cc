...
std::vector<std::vector<int64_t>> ConvAddMulAddHardSwishMulAddOpInferShape(
  ...
  VLOG(6)
      << "---------- here is ConvAddMulAddHardSwishMulAddOpInferShape ------";
...
std::vector<paddle::Tensor> ConvAddMulAddHardSwishMulAddOp(
  ...
  auto y1_tensor = static_cast<const phi::DenseTensor*>(y1.impl().get());
  ...
  auto y3_tensor = static_cast<const phi::DenseTensor*>(y3.impl().get());
  // Infershape
  auto out_shapes = ConvAddMulAddHardSwishMulAddOpInferShape(x.shape(),
                                                             filter.shape(),
                                                             y1.shape(),
                                                             x1.shape(),
                                                             y2.shape(),
  ...
  inputs["Y1"] = {const_cast<phi::DenseTensor*>(y1_tensor)};
  inputs["X1"] = {const_cast<phi::DenseTensor*>(x1_tensor)};
  inputs["Y2"] = {const_cast<phi::DenseTensor*>(y2_tensor)};
  inputs["X2"] = {const_cast<phi::DenseTensor*>(x2_tensor)};
  inputs["Y3"] = {const_cast<phi::DenseTensor*>(y3_tensor)};
...
PD_BUILD_OP(GCUConvAddMulAddHardSwishMulAdd)
    .Inputs({"X", "Filter", "Y1", "X1", "Y2", "X2", "Y3"})
    .Outputs({"Out"})
    .Attrs({"data_format: std::string",
            ...
            "axis4: int",
            "axis5: int"})
    .SetKernelFn(PD_KERNEL(ConvAddMulAddHardSwishMulAddOp))
    .SetInferShapeFn(PD_INFER_SHAPE(ConvAddMulAddHardSwishMulAddOpInferShape));

std::vector<std::vector<int64_t>>
DepthwiseConvAddMulAddHardSwishMulAddOpInferShape(
    const std::vector<int64_t>& x_shape,
    const std::vector<int64_t>& filter_shape,
    const std::vector<int64_t>& Y1,
    const std::vector<int64_t>& X1,
...
std::vector<paddle::Tensor> DepthwiseConvAddMulAddHardSwishMulAddOp(
    const paddle::Tensor& x,
    const paddle::Tensor& filter,
    const paddle::Tensor& y1,
    const paddle::Tensor& x1,
  ...
  // Infershape
  auto out_shapes =
      DepthwiseConvAddMulAddHardSwishMulAddOpInferShape(x.shape(),
                                                        filter.shape(),
                                                        y1.shape(),
                                                        x1.shape(),
                                                        y2.shape(),
...
std::vector<std::vector<int64_t>> DepthwiseConvAddMulAddOpInferShape(
    const std::vector<int64_t>& x_shape,
    const std::vector<int64_t>& filter_shape,
    const std::vector<int64_t>& Y1,
    const std::vector<int64_t>& X1,
    ...
    std::vector<int> paddings,
    std::vector<int> dilations) {
  VLOG(6) << "---------- here is DepthwiseConvAddMulAddOpInferShape ------";
  auto in_dims = phi::make_ddim(x_shape);
  auto filter_dims = phi::make_ddim(filter_shape);
  int dilation_size = dilations.size();
  for (int i = 0; i < dilation_size; ++i) {
...
std::vector<paddle::Tensor> DepthwiseConvAddMulAddOp(
    const paddle::Tensor& x,
    const paddle::Tensor& filter,
    const paddle::Tensor& y1,
    const paddle::Tensor& x1,
  ...
  auto y2_tensor = static_cast<const phi::DenseTensor*>(y2.impl().get());
  // Infershape
  auto out_shapes = DepthwiseConvAddMulAddOpInferShape(x.shape(),
                                                       filter.shape(),
                                                       y1.shape(),
                                                       x1.shape(),
                                                       y2.shape(),
  ...
  custom_kernel::GcuRunner(input_names,
                           ...
                           outputs,
                           attrs,
                           "depthwise_conv_add_mul_add",
                           *dev_ctx);

  return {out};
}

PD_BUILD_OP(GCUDepthwiseConvAddMulAddHardSwishMulAdd)
    .Inputs({"X", "Filter", "Y1", "X1", "Y2", "X2", "Y3"})
    .Outputs({"Out"})
    .Attrs({"data_format: std::string",
            "groups: int64_t",
            ...
            "axis4: int",
            "axis5: int"})
    .SetKernelFn(PD_KERNEL(DepthwiseConvAddMulAddHardSwishMulAddOp))
    .SetInferShapeFn(
        PD_INFER_SHAPE(DepthwiseConvAddMulAddHardSwishMulAddOpInferShape));

PD_BUILD_OP(GCUDepthwiseConvAddMulAdd)
    .Inputs({"X", "Filter", "Y1", "X1", "Y2"})
    .Outputs({"Out"})
    .Attrs({"data_format: std::string",
            "groups: int64_t",
            "padding_algorithm: std::string",
            ...
            "axis2: int",
            "axis3: int"})
    .SetKernelFn(PD_KERNEL(DepthwiseConvAddMulAddOp))
    .SetInferShapeFn(
        PD_INFER_SHAPE(DepthwiseConvAddMulAddHardSwishMulAddOpInferShape));
