...
std::vector<std::vector<int64_t>> ConvBnHardSwishOpInferShape(
  ...
  VLOG(3) << "---------- here is conv_bn_hard_swish ------";
...
std::vector<paddle::Tensor> ConvBnHardSwishOp(const paddle::Tensor& x,
  ...
  // Infershape
  auto out_shapes = ConvBnHardSwishOpInferShape(x.shape(),
                                                filter.shape(),
                                                mean.shape(),
                                                bias.shape(),
                                                scale.shape(),
...
PD_BUILD_OP(GCUConvBnHardSwish)
    ...
    .SetKernelFn(PD_KERNEL(ConvBnHardSwishOp))
    .SetInferShapeFn(PD_INFER_SHAPE(ConvBnHardSwishOpInferShape));
