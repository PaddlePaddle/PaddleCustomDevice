...
void BatchNormKernel(const Context& dev_ctx,
  ...
  VLOG(4) << "running_mean == mean_out : "
          << ((running_mean.data() == mean_out->data()) ? "true" : "false")
          << " running_mean place: " << running_mean.place()
          << " data: " << running_mean.data()
          << " mean_out place: " << mean_out->place()
          << " data: " << mean_out->data();

  VLOG(4) << "running_var == variance_out : "
          << ((running_var.data() == variance_out->data()) ? "true" : "false")
          << " running_var place: " << running_var.place()
          << " data: " << running_var.data()
          << " variance_out place: " << variance_out->place()
          << " data: " << variance_out->data();
  ...
  if (UseScatterMemory()) {
    ...
    auto scale_gcu = GetHlirTensor(scale);
    auto bias_gcu = GetHlirTensor(bias);
...
void BatchNormGradKernel(
  ...
  if (UseScatterMemory()) {
    ...
    if (scale_grad) {
      ...
      scale_grad_tmp.set_meta(scale.meta());
    ...
    if (bias_grad) {
      ...
      bias_grad_tmp.set_meta(bias.meta());
