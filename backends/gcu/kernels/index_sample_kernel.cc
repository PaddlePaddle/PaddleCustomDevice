...
void IndexSampleKernel(const Context &dev_ctx,
  ...
  if (LaunchAOTKernel()) {
    DenseTensor input_index = MaybeCreateOrTrans64To32bits(dev_ctx, index);
    bool sparse_grad = false;
    int64_t axis = (x.dims().size() > 1) ? (x.dims().size() - 1) : 0;
    LAUNCH_TOPSATENOP(
        topsatenGather, dev_ctx, *out, x, axis, input_index, sparse_grad);
    // auto index_type = index.dtype();
    // bool index_type_match = index_type == DataType::INT32 ||
    //                         index_type == DataType::INT64;
    // PADDLE_ENFORCE_EQ(index_type_match, true,
    //                   phi::errors::InvalidArgument(
    //                       "Input(Index) holds the wrong type, it holds %s,
    //                       but " "desires to be %s or %s",
    //                       DataTypeToString(index_type).c_str(),
    //                       DataTypeToString(DataType::INT32).c_str(),
    //                       DataTypeToString(DataType::INT64).c_str()));
