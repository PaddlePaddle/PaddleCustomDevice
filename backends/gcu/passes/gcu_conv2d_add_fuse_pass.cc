...
class GcuConv2dAddFusePattern : public paddle::drr::DrrPatternBase {
 private:
  std::string conv2d_op_name_;

 public:
  std::string name() const override { return "GcuConv2dAddFusePattern"; }

  explicit GcuConv2dAddFusePattern(const std::string &conv2d_op_name)
...
class GcuConv2dAddFusePass : public pir::PatternRewritePass {
 public:
  GcuConv2dAddFusePass()
      : pir::PatternRewritePass("gcu_conv2d_add_fuse_pass", 2) {}

  pir::RewritePatternSet InitializePatterns(pir::IrContext *context) override {
    pir::RewritePatternSet ps(context);
    ps.Add(paddle::drr::Create<GcuConv2dAddFusePattern>(
...
namespace pir {
std::unique_ptr<Pass> CreateGcuConv2dAddFusePass() {
  return std::make_unique<GcuConv2dAddFusePass>();
}
}  // namespace pir

REGISTER_IR_PASS(gcu_conv2d_add_fuse_pass, GcuConv2dAddFusePass);
