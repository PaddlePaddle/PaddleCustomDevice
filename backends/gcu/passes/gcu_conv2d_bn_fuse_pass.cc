...
class GcuDepthwiseConv2dBnFusePattern
    : public pir::OpRewritePattern<paddle::dialect::BatchNorm_Op> {
 public:
  using pir::OpRewritePattern<paddle::dialect::BatchNorm_Op>::OpRewritePattern;
  bool MatchAndRewrite(
    ...
    paddle::dialect::DepthwiseConv2dOp depthwise_conv2d_op =
        pir::GetDefiningOpForInput(op, 0)
            ->dyn_cast<paddle::dialect::DepthwiseConv2dOp>();
    if (!depthwise_conv2d_op) return false;

    auto conv2d_attributes = depthwise_conv2d_op->attributes();
    auto padding_algorithm = conv2d_attributes.at("padding_algorithm")
                                 .dyn_cast<pir::StrAttribute>()
                                 .AsString();
    if (padding_algorithm != "EXPLICIT" && padding_algorithm != "SAME" &&
    ...
    if (!depthwise_conv2d_op.out().HasOneUse()) return false;
    // (bukejiyu): The bn
    // outputs(mean_out\variance_out\saved_mean\saved_variance)
    //  cannot be used in conv bn fusion
    if (!op.mean_out().use_empty()) return false;
    ...
    pir::Value conv2d_filter = depthwise_conv2d_op.filter();
    pir::Value bn_mean = op.mean();
    pir::Value bn_variance = op.variance();
    pir::Value bn_scale = op.scale();
    pir::Value bn_bias = op.bias();
    ...
    paddle::dialect::ReshapeOp reshape_scale_op =
        rewriter.Build<paddle::dialect::ReshapeOp>(div_op.out(),
                                                   bn_scale_new_shape);
    paddle::dialect::DepthwiseConv2dOp new_depthwise_conv2d_op;
    auto conv2d_filter_dtype = pir::GetDataTypeFromValue(conv2d_filter);
    if (conv2d_filter_dtype.isa<pir::Float16Type>()) {
      auto cast_in_op = rewriter.Build<paddle::dialect::CastOp>(
          conv2d_filter, phi::DataType::FLOAT32);
      ...
      auto cast_out_op = rewriter.Build<paddle::dialect::CastOp>(
          mul_op.out(), phi::DataType::FLOAT16);
      new_depthwise_conv2d_op =
          rewriter.Build<paddle::dialect::DepthwiseConv2dOp>(
              depthwise_conv2d_op.input(),
              cast_out_op.out(),
              conv2d_attributes);
    } else {
      auto mul_op = rewriter.Build<paddle::dialect::MultiplyOp>(
      ...
      new_depthwise_conv2d_op =
          rewriter.Build<paddle::dialect::DepthwiseConv2dOp>(
              depthwise_conv2d_op.input(), mul_op.out(), conv2d_attributes);
    }
    ...
    paddle::dialect::SubtractOp sub_op =
        rewriter.Build<paddle::dialect::SubtractOp>(bn_bias, mul_bias_op.out());
    // reshape new bias
    auto new_conv2d_out_shape =
        pir::GetShapeFromValue(new_depthwise_conv2d_op.out());
    std::vector<int64_t> new_bias_new_shape(new_conv2d_out_shape.size(), 1);
    new_bias_new_shape[1] = new_conv2d_out_shape[1];
    paddle::dialect::ReshapeOp reshape_bias_op =
        rewriter.Build<paddle::dialect::ReshapeOp>(sub_op.out(),
    ...
    if (conv2d_filter_dtype.isa<pir::Float16Type>()) {
      auto cast_op = rewriter.Build<paddle::dialect::CastOp>(
          reshape_bias_op.out(), phi::DataType::FLOAT16);
      add_bias_op = rewriter.Build<paddle::dialect::AddOp>(
          new_depthwise_conv2d_op.out(), cast_op.out());
    } else {
      add_bias_op = rewriter.Build<paddle::dialect::AddOp>(
          new_depthwise_conv2d_op.out(), reshape_bias_op.out());
    }

    rewriter.ReplaceAllUsesWith(op.out(), add_bias_op.out());

    rewriter.EraseOp(op);
    rewriter.EraseOp(depthwise_conv2d_op);
    return true;
  }
};

class GcuBatchNormReplacePattern
...
class GcuConv2dBnFusePass : public pir::PatternRewritePass {
 public:
  GcuConv2dBnFusePass()
      : pir::PatternRewritePass("gcu_conv2d_bn_fuse_pass", 2) {}

  pir::RewritePatternSet InitializePatterns(pir::IrContext *context) override {
    pir::RewritePatternSet ps(context);
    auto conv_bn_pattern = std::make_unique<GcuConv2dBnFusePattern>(
        context,
        1,
    ...
    auto depthwise_conv_bn_pattern =
        std::make_unique<GcuDepthwiseConv2dBnFusePattern>(
            context,
            1,
            std::vector<std::string>{
                ...
                paddle::dialect::MultiplyOp::name(),
                paddle::dialect::SubtractOp::name(),
                paddle::dialect::DepthwiseConv2dOp::name()});
    auto bn_replace_pattern = std::make_unique<GcuBatchNormReplacePattern>(
        context,
        1,
    ...
    // depthwise_conv2d + bn -> depthwise_conv2d
    ps.Add(std::move(depthwise_conv_bn_pattern));
    return ps;
  }
};
...
std::unique_ptr<Pass> CreateGcuConv2dBnFusePass() {
  return std::make_unique<GcuConv2dBnFusePass>();
}

}  // namespace pir

REGISTER_IR_PASS(gcu_conv2d_bn_fuse_pass, GcuConv2dBnFusePass);
