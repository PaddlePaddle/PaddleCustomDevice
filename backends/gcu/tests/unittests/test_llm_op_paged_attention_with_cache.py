...
PAGED_ATTENTION_V1_CASE = [
    ...
    {"num_seqs": 4, "num_heads": 4, "num_kv_heads": 4, "head_size": 128, "block_size": 16, "max_tokens_size": 2048, "dtype": np.float16},
    {"num_seqs": 5, "num_heads": 4, "num_kv_heads": 4, "head_size": 128, "block_size": 16, "max_tokens_size": 2048, "dtype": np.float16},
    {"num_seqs": 10, "num_heads": 64, "num_kv_heads": 8, "head_size": 128, "block_size": 16, "max_tokens_size": 2048, "dtype": np.float16},
    {"num_seqs": 16, "num_heads": 64, "num_kv_heads": 8, "head_size": 128, "block_size": 16, "max_tokens_size": 2048, "dtype": np.float16},
    {"num_seqs": 16, "num_heads": 64, "num_kv_heads": 8, "head_size": 128, "block_size": 32, "max_tokens_size": 2048, "dtype": np.float16},
    {"num_seqs": 16, "num_heads": 64, "num_kv_heads": 8, "head_size": 256, "block_size": 32, "max_tokens_size": 2048, "dtype": np.float16},
...
def native_attention_impl(
    ...
    Args:
        ...
        key_cache:    [num_blocks, num_kv_heads, head_size/x, block_size, x] or
                      [num_kv_tokens, num_kv_heads, head_size]
        value_cache:  [num_blocks, num_kv_heads, head_size, block_size] or
                      [num_kv_tokens, num_kv_heads, head_size]
        seq_lens:     [num_seqs]
    ...
    if from_cache:
        ...
        for i in range(num_seqs):
            ...
            # print(f"TESTCASE_DEBUG In native_attention_impl, q.shape:{q.shape}, "
            #       f"keys.shape:{keys.shape}, values.shape:{values.shape}", flush=True)
        ...
        for i in range(num_seqs):
            ...
            k = key_cache[kv_start_idx : kv_start_idx + seq_len, :, :]
            v = value_cache[kv_start_idx : kv_start_idx + seq_len, :, :]

            # print(f"TESTCASE_DEBUG In native_attention_impl, q.shape:{q.shape}, "
            #       f"k.shape:{k.shape}, v.shape:{v.shape}", flush=True)

            # inputs: [num_tokens, num_heads, head_size] -> [1, num_tokens, num_heads, head_size]
            out = run_sdpa(q.unsqueeze(0), k.unsqueeze(0), v.unsqueeze(0))
            outputs.append(out.squeeze(0))
            kv_start_idx += seq_len
...
class TestPagedAttentionV1(TestAPIBase):
    ...
    def prepare_data(self):
        ...
        self.num_kv_tokens = np.sum(self.seq_lens)
        ...
        self.key = self.generate_data(
            shape=[self.num_kv_tokens, self.num_kv_heads, self.head_size],
            dtype=self.dtype,
        )
        ...
        self.free_slots = np.arange(0, max_block_size + 1, dtype=np.int32)

        max_num_blocks_per_seq = (
            self.max_seq_len + self.block_size - 1
        ) // self.block_size
        self.block_tables = np.full(
            shape=[self.num_seqs, max_num_blocks_per_seq], fill_value=-1, dtype=np.int32
        )
        # print(f"TESTCASE_DEBUG In prepare_data, self.block_tables shape:{self.block_tables.shape}", flush=True)

        for i in range(self.num_seqs):
            select_index = self.free_slots[:max_num_blocks_per_seq]
            self.free_slots = self.free_slots[max_num_blocks_per_seq:]
            self.block_tables[i] = select_index
        # print(f"TESTCASE_DEBUG In prepare_data, self.block_tables:{self.block_tables}", flush=True)

        self.alloced_slots = np.zeros(
            shape=[
                self.num_seqs,
                max_num_blocks_per_seq * self.block_size,
            ],
            dtype=np.int32,
        )
        for i in range(self.num_seqs):
            blockes = self.block_tables[i]
            for j in range(max_num_blocks_per_seq):
                block_idx = blockes[j]
                idx = j * self.block_size
                self.alloced_slots[i][idx : idx + self.block_size] = np.arange(
                    block_idx * self.block_size,
                    (block_idx + 1) * self.block_size,
                    dtype=np.int32,
                )
        # print(f"TESTCASE_DEBUG In prepare_data, self.alloced_slots:{self.alloced_slots}", flush=True)

    def alloc_slots(self, kv_lens, pre_lens=None):
        num_tokens = np.sum(kv_lens)
        slots = np.zeros(shape=[num_tokens], dtype=np.int32)
        start_idx = 0
        for i in range(len(kv_lens)):
            alloced_slots_start = pre_lens[i] if pre_lens is not None else 0
            slots[start_idx : start_idx + kv_lens[i]] = self.alloced_slots[i][
                alloced_slots_start : alloced_slots_start + kv_lens[i]
            ]
            start_idx += kv_lens[i]
        # print(f"TESTCASE_DEBUG In alloc_slots, return slots:{slots}", flush=True)
        return slots
    ...
    def forward(self):
        ...
        # 1. write cache
        alloc_slots = self.alloc_slots(self.seq_lens)
        slot_mapping = paddle.to_tensor(alloc_slots, dtype="int32")
        ...
        self.key_cache_update = key_cache.clone()
        self.value_cache_update = value_cache.clone()
    ...
    def ref_attention_impl(self, dtype):
        ...
        key_cache = paddle.to_tensor(self.key, dtype=dtype)
        value_cache = paddle.to_tensor(self.value, dtype=dtype)
        if from_cache:
            key_cache = self.key_cache_update.to(NATIVE_IMPL_DEV).astype(dtype)
            value_cache = self.value_cache_update.to(NATIVE_IMPL_DEV).astype(dtype)
