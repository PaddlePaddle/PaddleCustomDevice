...
class ApiBase(object):
    def __init__(
        ...
        # TODO(wangzhengjun) gcu300 only support inference now
        ...
        with paddle.utils.unique_name.guard():
            with paddle.static.program_guard(
                ...
                def input_data(name, shape, dtype):
                    data = paddle.static.data(name=name, shape=shape, dtype=dtype)
                ...
                for i in range(len(self.feed_list)):
                    ...
                    # else:
                    data = input_data(
                        self.feed_list[i], self.feed_shapes[i], self.feed_dtypes[i]
                    )
                    inputs.append(data)
                out = None
                ...
                if is_tuple_list(out):
                    for item in out:
                        if is_tuple_list(item):
                            fetch_list.append([mini_item.name for mini_item in item])
                        else:
