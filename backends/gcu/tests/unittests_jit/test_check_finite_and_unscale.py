...
def compare(cpu_res, gcu_res, threshold=1.0e-5, rel_tol=1.0e-6, equal_nan=False):
...
class TestCheckFiniteAndUnscale(unittest.TestCase):
    def get_prog(self, N):
        ...
        with program_guard(main_program):
            A = []
            for i in range(N):
                a = paddle.static.data(name="c%d" % i, shape=[32, 32], dtype="float32")
                A.append(a)
    ...
    def run_prog(self, c, scale, nonan=True):
        ...
        for i in range(len(c)):
            vs["c%d" % i] = c[i]
        vs["scale"] = scale
        out_, founf_inf_ = exe.run(main_program, feed=vs, fetch_list=[out, found_inf])
