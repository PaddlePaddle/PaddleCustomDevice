...
def check_state_dict(state_dict, dist_state_dict):
    ...
    for key in sorted(state_dict):
        tensor = paddle.base.executor.as_numpy(state_dict[key])
        dist_tensor = paddle.base.executor.as_numpy(dist_state_dict[key])
        assert np.allclose(
...
def distributed_test(batch_size=8, total_steps=10, compare_with_cpu=False):
    ...
    with paddle.static.scope_guard(single_run_scope):
        ...
        if not compare_with_cpu:
            res = gcu_exe.run(main_prog, feed=input_datas[0], fetch_list=output_list)
        else:
            res = cpu_exe.run(main_prog, feed=input_datas[0], fetch_list=output_list)
        single_run_state = main_prog.state_dict()

    # distributed
    dist_run_scope = paddle.static.Scope()
    with paddle.static.scope_guard(dist_run_scope):
        dist_startup_prog, dist_main_prog = build_program(dist_mini_batch, seed, True)
