...
def test_cross_entropy():
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[4, 1000], dtype="float32")
            label = paddle.static.data(name="label", shape=[4, 1000], dtype="float32")
            data.stop_gradient = False
            label.stop_gradient = True
            loss = paddle.nn.functional.cross_entropy(data, label, soft_label=True)
            ...
            x2 = np.random.uniform(0.1, 1.0, (4, 1000)).astype("float32")
            output = exe.run(
                main_program, feed={"data": x, "label": x2}, fetch_list=fetch_list
            )
