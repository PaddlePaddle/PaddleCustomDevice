...
def test_dropout():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[2, 3, 16], dtype="float32")
...
def test_dropout2():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[4, 4], dtype="float32")
            data.stop_gradient = False
            out = paddle.paddle.nn.functional.dropout(data, p=0.5, training=False)
            feed_list = ["data"]
            fetch_list = [out.name]
...
def test_dropout_f64():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[2, 3, 16], dtype="float64")
            data.stop_gradient = False
            out = paddle.nn.functional.dropout(data, p=0.4)
            loss = paddle.mean(out)
            fetch_list = [out.name]
