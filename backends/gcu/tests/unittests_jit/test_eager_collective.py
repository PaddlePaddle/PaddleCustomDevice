...
def train(model, dist_model, data_loader, world_size):
    optim = paddle.optimizer.Adam(
        learning_rate=0.001, parameters=model.parameters(), use_multi_tensor=True
    )
    dist_optim = paddle.optimizer.Adam(
        learning_rate=0.001, parameters=dist_model.parameters(), use_multi_tensor=True
    )
    rank = dist.get_rank()
    ...
    for epoch in range(EPOCH_NUM):
        ...
        for batch_id, data in enumerate(data_loader()):
            ...
            if rank == 0:
                print("Train successfully, epoch:{}, step:{}".format(epoch, batch_id))
