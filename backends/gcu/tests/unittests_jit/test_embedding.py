...
def test_embedding():
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[6, 128], dtype="int64")
            ...
            output = exec.run(main_program, feed={"data": x}, fetch_list=fetch_list)
            print("cpu run success")
            exe = paddle.static.Executor("gcu:0")
            output_gcu = exe.run(main_program, feed={"data": x}, fetch_list=fetch_list)
            print("gcu run success!")
            print("output num:", len(output))
            for i in range(len(output)):
                print("------------------------------------------")
                print(cosSim(output[i], output_gcu[i]))
                print(np.allclose(output[i], output_gcu[i], atol=1e-5, rtol=1e-5))
