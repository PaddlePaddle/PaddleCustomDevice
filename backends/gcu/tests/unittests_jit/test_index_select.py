...
def test_index_select_with_grad_rank3_axis_1():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[2, 3, 4], dtype="float64")
            ...
            exe_gcu = paddle.static.Executor("gcu:0")
            ...
            for i in range(len(output_cpu)):
                ...
                assert np.allclose(output_gcu[i], output_cpu[i], atol=1e-5, rtol=1e-5)
...
def test_index_select_with_grad_rank4_axis0():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[2, 3, 2, 2], dtype="int64")
            index = paddle.static.data(name="index", shape=[2], dtype="int32")
            data.stop_gradient = False
            index.stop_gradient = True
...
def test_index_select_with_grad_rank4_axis2():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[2, 3, 2, 2], dtype="float32")
            index = paddle.static.data(name="index", shape=[2], dtype="int32")
            data.stop_gradient = False
            index.stop_gradient = True
            ...
            print(fetch_list)
            print(main_program)
            feed_data = np.random.uniform(0.1, 1.0, (2, 3, 2, 2)).astype("float32")
            feed_index = np.array([1, 1], dtype="int32")
            cpu_place = paddle.CPUPlace()
            exe_cpu = paddle.static.Executor(cpu_place)
            exe_cpu.run(startup_program)
...
def test_index_select_with_grad_rank4_axis2_float64():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[2, 3, 2, 2], dtype="float64")
            index = paddle.static.data(name="index", shape=[2], dtype="int32")
            data.stop_gradient = False
            index.stop_gradient = True
            loss = paddle.index_select(x=data, index=index, axis=2)
            ...
            print(fetch_list)
            print(main_program)
            feed_data = np.random.uniform(0.1, 1.0, (2, 3, 2, 2)).astype("float64")
            feed_index = np.array([1, 1], dtype="int32")
            cpu_place = paddle.CPUPlace()
            exe_cpu = paddle.static.Executor(cpu_place)
            exe_cpu.run(startup_program)
