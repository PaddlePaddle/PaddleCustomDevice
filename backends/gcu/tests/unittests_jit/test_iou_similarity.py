...
def _compare(cpu_res, gcu_res):
    ...
    for i in range(len(cpu_res)):
        out = gcu_res[i]
        exp = cpu_res[i]
        assert out.shape == exp.shape, f"out shape: {out.shape}, expect: {exp.shape}"
...
def _calc_output(x, y, box_normalized=True):
    ...
    print("PROG_DEBUG main_program:{}".format(main_program))
    print(
        "cpu_res shape:{}, gcu_res shape:{}".format(cpu_res[0].shape, gcu_res[0].shape)
    )
    print("PROG_DEBUG cpu_res:{}".format(cpu_res))
    print("PROG_DEBUG gcu_res:{}".format(gcu_res))
