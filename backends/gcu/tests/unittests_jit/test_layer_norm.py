...
def test_layer_norm_0():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            weight3 = paddle.static.data(name="weight3", shape=[2048], dtype="float32")
            bias3 = paddle.static.data(name="bias3", shape=[2048], dtype="float32")
            ...
            for i in range(len(output)):
                print("-----------------------------")
                print("cossim is: {}".format(cosSim(output[i], output_cpu[i])))
...
def test_layer_norm_1():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            data = paddle.static.data(name="data", shape=[2, 2, 2, 3], dtype="float32")
            weight3 = paddle.static.data(name="weight3", shape=[3], dtype="float32")
            bias3 = paddle.static.data(name="bias3", shape=[3], dtype="float32")
            data.stop_gradient = False
            weight3.stop_gradient = False
            bias3.stop_gradient = False
            out = paddle.nn.functional.layer_norm(data, 3, weight=weight3, bias=bias3)
            loss = paddle.mean(out)
            fetch_list = ["data"]
            fetch_list = [loss.name, out.name]
            g = paddle.static.gradients(loss, [data, weight3, bias3])
...
def test_layer_norm_2():
    ...
    with paddle.utils.unique_name.guard():
        with paddle.static.program_guard(
            ...
            exec = paddle.static.Executor(paddle.CPUPlace())
            exec.run(startup_program)
            output = exe.run(main_program, feed={"data": x}, fetch_list=fetch_list)
            output_cpu = exec.run(main_program, feed={"data": x}, fetch_list=fetch_list)
            print("output num:", len(output))
            for i in range(len(output)):
                print("-----------------------------")
                print("cossim is: {}".format(cosSim(output[i], output_cpu[i])))
