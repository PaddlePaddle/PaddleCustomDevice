...
def make_rois(
    batch_size, input_height, input_width, pooled_height, pooled_width, spatial_scale
):
    rois = []
    rois_lod = [[]]
...
def test_roi_align():
    ...
    roi = paddle.base.create_lod_tensor(rois[:, 1:5], rois_lod, paddle.CPUPlace())
    ...
    test.run(
        feed=[data, roi, boxes_num], output_size=1, spatial_scale=0.1, sampling_ratio=1
    )
