...
def my_squared_l2_norm(input, name=None):
    ...
    check_variable_and_dtype(input, "x", ["float32", "float64"], "squared_l2_norm")
    out = helper.create_variable_for_type_inference(dtype=input.dtype)
    helper.append_op(type="squared_l2_norm", inputs={"X": input}, outputs={"Out": out})
    return out
...
test2 = ApiBase(
    func=my_squared_l2_norm, feed_names=["data"], feed_shapes=[[1]], is_train=False
)
