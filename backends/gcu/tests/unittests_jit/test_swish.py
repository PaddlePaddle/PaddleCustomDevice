...
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_swish_4():
    swish_test_with_random_data(paddle.nn.functional.swish, shape=[128, 672, 15, 15])


@pytest.mark.swish
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_swish_5():
    swish_test_with_random_data(paddle.nn.functional.swish, shape=[128, 240, 30, 30])


@pytest.mark.swish
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_swish_6():
    swish_test_with_random_data(paddle.nn.functional.swish, shape=[128, 10, 1, 1])


@pytest.mark.swish
@pytest.mark.filterwarnings("ignore::UserWarning")
def test_swish_7():
    swish_test_with_random_data(paddle.nn.Swish, shape=[128, 8, 1, 1], dtype="float64")
