...
class TestBatchNormOpTraining(unittest.TestCase):
    ...
    def test_forward_backward(self):
        ...
        for data_format in self.data_formats:
            if data_format == "NHWC":
                continue
            test_with_place(paddle.CustomPlace("gcu", 0), data_format, [2, 3, 4, 5])
            test_with_place(paddle.CustomPlace("gcu", 0), data_format, [3, 8, 5])
            test_with_place(paddle.CustomPlace("gcu", 0), data_format, [2, 3])
...
class TestDygraphBatchNormTrainableStats(unittest.TestCase):
    def test_dygraph(self):
        ...
        y2 = compute(x, False, False)
        y3 = compute(x, True, True)
        y4 = compute(x, True, True)
    ...
    def test_static(self):
        exe = base.Executor(paddle.CustomPlace("gcu", 0))
        shape = [4, 10, 16, 16]
        ...
        def compute_storage(x_np, is_test, trainable_statistics):
            with program_guard(Program(), Program()):
                bn = paddle.nn.BatchNorm(
        ...
        y3 = compute_storage(x, False, False)
        y4 = compute_storage(x, True, True)
