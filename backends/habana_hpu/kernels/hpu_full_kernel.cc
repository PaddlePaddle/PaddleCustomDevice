...
class FullOperator : public HpuOperator {
  ...
  void AddNode(const std::vector<DIMS>& ins,
              const std::vector<DIMS>& outs, VType val) {
    ...
    synStatus status = synNodeCreate(graphHandle_, nullptr, outputs, 0, outs.size(), &params, sizeof(params), "constant_f32", "constant_fwd_op", nullptr, nullptr);
