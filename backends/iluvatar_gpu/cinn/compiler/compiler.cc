...
// PaddleCustomDevice/backends/iluvatar_gpu/cinn/compiler/compiler.cc
...
C_Status IluvatarCompile(void* dev_ptr,
  ...
  bool keep_tmp = (std::getenv("KEEP_TMP") != nullptr);
  ...
  auto cleanup = [&](bool success) {
    if (!keep_tmp) {
      ...
      if (!success) {
        std::remove(cubin_path.c_str());  // 编译失败时也删 .cubin
      }
    }
  };
