...
C_Status IluvatarModuleLoad(void* dev_ptr, const char* path, void** mod_out) {
  CUmodule module;
...
// 【实现2】卸载模块
C_Status IluvatarModuleUnload(void* dev_ptr, void* module_handle) {
  cuModuleUnload((CUmodule)module_handle);
  return C_Status::C_SUCCESS;
}

// 【实现3】获取函数地址：相当于 cudaModuleGetFunction
C_Status IluvatarGetKernelAddress(void* dev_ptr,
                                  void* module_handle,
...
C_Status IluvatarLaunchKernel(void* dev_ptr,
                              void* func_ptr,
                              void** args,
                              int num_args,
                              int gx,
