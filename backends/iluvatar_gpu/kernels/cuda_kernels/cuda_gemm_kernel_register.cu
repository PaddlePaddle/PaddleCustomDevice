...
__device__ int __dp4a(int8_t a, int8_t b, int8_t acc) {
  acc += ((a & 0xFF) * (b & 0xFF));
  acc += (((a >> 8) & 0xFF) * ((b >> 8) & 0xFF));
  acc += (((a >> 16) & 0xFF) * ((b >> 16) & 0xFF));
  acc += (((a >> 24) & 0xFF) * ((b >> 24) & 0xFF));
  return acc;
}
...
PD_CUSTOM_KERNEL_REGISTER(
    cuda_gemm, iluvatar_gpu, ALL_LAYOUT, phi::CudaGemm, int8_t) {}
