...
void SolveLinearSystemGPU<phi::dtype::complex<float>>(
  ...
  cusolverDnHandle_t cusolver_handle = GetSolverHandle(dev_ctx.stream());
...
void ComputeBackwardForComplexInputGPU(const DenseTensor& L,
  ...
  phi::Copy(dev_ctx, diag_res_cpu, dev_ctx.GetPlace(), false, &diag_res);
...
// Register the kernel
PD_REGISTER_PLUGIN_KERNEL(eig_grad,
                          iluvatar_gpu,
                          ALL_LAYOUT,
                          phi::EigGradKernel,
                          float,
