...
void FlashAttnUnpaddedGradBaseKernel(
  ...
  flashAttnInfo.philox_args           = *(reinterpret_cast<cudnnPhiloxCudaState *>(&philox_state));
  flashAttnInfo.imp_mode              = FLAGS_ixdnn_imp_mode ? CUDNN_FATTN_LEAST_MEM_MODE : CUDNN_FATTN_BALANCE_MODE;
  ...
  std::vector<int32_t> qShape, kShape, vShape, oShape, lseShape, dqShape, dkShape, dvShape, doShape;
  std::vector<int32_t> qStride, kStride, vStride, oStride, lseStride, dqStride, dkStride, dvStride, doStride;
  ...
  cudnnTensorDescriptor_t q_desc, k_desc, v_desc, o_desc, lse_desc, dq_desc, dk_desc, dv_desc, do_desc;

  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateFlashAttnDescriptor(&flashAttnDesc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&q_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&k_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&v_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&o_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&lse_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&dq_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&dk_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&dv_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&do_desc));
  ...
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(q_desc, dataType, nb_dims, qShape.data(), qStride.data()));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(k_desc, dataType, nb_dims, kShape.data(), kStride.data()));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(v_desc, dataType, nb_dims, vShape.data(), vStride.data()));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(o_desc, dataType, nb_dims, oShape.data(), oStride.data()));
  if (is_unpad) {
    PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor_lowerbound_2(lse_desc, CUDNN_DATA_FLOAT, nb_dims - 1, lseShape.data(), lseStride.data()));
  } else {
    PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(lse_desc, CUDNN_DATA_FLOAT, nb_dims - 1, lseShape.data(), lseStride.data()));
  }
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(dq_desc, dataType, nb_dims, dqShape.data(), dqStride.data()));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(dk_desc, dataType, nb_dims, dkShape.data(), dkStride.data()));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(dv_desc, dataType, nb_dims, dvShape.data(), dvStride.data()));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(do_desc, dataType, nb_dims, doShape.data(), doStride.data()));

  size_t size_tmpbuf = 0;
  void*    d_wkSpace   = nullptr;

  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnGetFlashAttnBuffers(cudnn,
                                      flashAttnDesc,
                                      batch_size,
                                      num_heads,
                                      max_seqlen_q,
  ...
  CUDA_CHECK(cudaMalloc((void**)&d_wkSpace, size_tmpbuf));
  CUDA_CHECK(cudaMemset(d_wkSpace, 0xff, size_tmpbuf));

  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnFlashAttnBackward(cudnn,
                                    flashAttnDesc,
                                    flashAttnInfo,
                                    q_desc,
                                    k_desc,
    ...
    if (!is_mha) {
      ...
      if (dk) {
        if (is_unpad)
          dk_tmp.Resize({total_k, num_heads_k, num_heads / num_heads_k, head_size});
        else
          dk_tmp.Resize({batch_size, max_seqlen_k, num_heads_k, num_heads / num_heads_k, head_size});
        if (dk->meta().is_contiguous())
          phi::SumKernel<T, Context>(ctx, dk_tmp, {reduce_axis}, dk->type(), false, dk);
        else
          kvReduceForGQA<T, Context>(ctx, dk_tmp, dk);
      }
      if (dv) {
        if (is_unpad)
          dv_tmp.Resize({total_k, num_heads_k, num_heads / num_heads_k, head_size});
        else
          dv_tmp.Resize({batch_size, max_seqlen_k, num_heads_k, num_heads / num_heads_k, head_size});
        if (dv->meta().is_contiguous())
          phi::SumKernel<T, Context>(ctx, dv_tmp, {reduce_axis}, dv->type(), false, dv);
        else
  ...
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnDestroyTensorDescriptor(q_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnDestroyTensorDescriptor(k_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnDestroyTensorDescriptor(v_desc));
  PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnDestroyTensorDescriptor(o_desc));
...
template <typename T, typename Context>
void FlashAttnUnpaddedGradKernel_(const Context& ctx,
                                 const DenseTensor& q,
                                 const DenseTensor& k,
                                 const DenseTensor& v,
                                 const DenseTensor& cu_seqlens_q,
...
void FlashAttnGradBaseKernel(
  ...
  const float softmax_unscale = std::sqrt(head_size);
    ...
    cudnnFlashAttnDescriptor_t flashAttnDesc;
    cudnnTensorDescriptor_t q_desc, k_desc, v_desc, o_desc, m_desc, lse_desc, dq_desc, dk_desc, dv_desc, do_desc;
    ...
    if (attn_mask.get_ptr()) {
      ...
      PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnCreateTensorDescriptor(&m_desc));
      copyDimsAndStrides(attn_mask.get(), mShape, mStride);
      PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnSetTensorNdDescriptor(m_desc, dataType, nb_dims, mShape.data(), mStride.data()));
    }
    ...
    if (!is_mha) {
      if (dk) {
        dk_tmp.Resize({batch_size, seqlen_k, num_heads_k, num_heads / num_heads_k, head_size_og});
        if (dk->meta().is_contiguous())
      ...
      if (dv) {
        dv_tmp.Resize({batch_size, seqlen_k, num_heads_k, num_heads / num_heads_k, head_size_og});
        if (dv->meta().is_contiguous())
