...
void FlashAttnUnpaddedBaseKernel(
  ...
  // [&](void* workspace_ptr) {
  //   PADDLE_ENFORCE_GPU_SUCCESS(phi::dynload::cudnnFlashAttnForward(cudnn,
  //                                   flashAttnDesc,
  //                                   flashAttnInfo,
  //                                   q_desc,
  //                                   k_desc,
...
void FlashAttnBaseKernel(
  ...
  PADDLE_ENFORCE_LE(head_size,
                    512,
                    phi::errors::InvalidArgument(
                      "FlashAttention forward only supports head dimension at most 512, but get head_dim=[head_size]"));
  PADDLE_ENFORCE_EQ(num_heads % num_heads_k,
                    0,
                    phi::errors::InvalidArgument(
...
PD_CUSTOM_KERNEL_REGISTER(flash_attn_unpadded,
                          iluvatar_gpu,
                          ALL_LAYOUT,
                          phi::FlashAttnUnpaddedKernel_,
                          phi::dtype::float16,
                          phi::dtype::bfloat16) {
  kernel->InputAt(5).SetBackend(phi::Backend::ALL_BACKEND);  // fixed_seed_offset
}
...
PD_CUSTOM_KERNEL_REGISTER(flash_attn,
                          ...
                          phi::dtype::float16,
                          phi::dtype::bfloat16) {
  kernel->InputAt(3).SetBackend(phi::Backend::ALL_BACKEND);  // fixed_seed_offset
}
