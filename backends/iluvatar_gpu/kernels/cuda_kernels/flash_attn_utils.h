...
inline void copyDimsAndStrides(const DenseTensor& tensor, std::vector<int32_t>& shape, std::vector<int32_t>& stride) {
...
inline void copyDimsAndStrides(DenseTensor* tensor, std::vector<int32_t>& shape, std::vector<int32_t>& stride) {
  for (size_t i = 0; i < tensor->dims().size(); ++i) {
    shape.push_back(static_cast<int>(tensor->dims()[i]));
    stride.push_back(static_cast<int>(tensor->strides()[i]));
  }
