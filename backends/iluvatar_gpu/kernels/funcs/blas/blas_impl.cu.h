...
inline void CublasCall(const std::function<void(cublasHandle_t)>& callback, phi::stream::stream_t stream) {
...
inline bool IsTensorCoreAvailable() {
  return blas_tensor_core_handle_ != nullptr;
}

inline void TensorCoreCublasCallIfAvailable(const std::function<void(cublasHandle_t)>& callback, phi::stream::stream_t stream) {
...
// here.
    ...
    VLOG(5) << "use_tensor_op_math: "
            << (IsTensorCoreAvailable() ? "True" : "False");
    TensorCoreCublasCallIfAvailable([&](cublasHandle_t handle) {
...
struct CUBlas<phi::dtype::float16> {
  ...
  static void GEMM_BATCH(phi::GPUContext *dev_ctx,
    ...
    bool use_tensor_op_math = IsTensorCoreAvailable();
...
void Blas<phi::GPUContext>::GEMM(CBLAS_TRANSPOSE transA,
  ...
  if (FLAGS_enable_cublas_tensor_op_math && std::is_same<T, float>::value) {
    ...
    CublasCall([&](cublasHandle_t handle) {
...
void Blas<phi::GPUContext>::SCAL(int n, const T alpha, T *x) const {
  CublasCall(
      [&](cublasHandle_t handle) { CUBlas<T>::SCAL(handle, n, &alpha, x, 1); }, context_.stream());
}
...
void Blas<phi::GPUContext>::VCOPY(int n, const T *x, T *y) const {
  CublasCall(
      [&](cublasHandle_t handle) { CUBlas<T>::VCOPY(handle, n, x, 1, y, 1); }, context_.stream());
}
