...
#include "fake_quantize_functor.h"
...
__global__ void LayerNormForward(
  ...
  __shared__ U shared_mean[WARP_SIZE];  // threadIdx.x / warpSize <= kMaxBlockDim /
                                 // warpSize <= 1024/32 = 32;
  __shared__ U shared_var[WARP_SIZE];
...
void ln_bwd_fast_kernel_driver(const phi::GPUContext &dev_ctx,
  ...
  if (cols == 1024 || cols == 512 || cols == 256) {
#else
...
#ifdef PADDLE_WITH_COREX
    const int BYTES_PER_LDG = 4;
    const int VecSize = std::max(1, static_cast<int>(BYTES_PER_LDG / sizeof(T)));
#else
...
#ifdef PADDLE_WITH_COREX
          case 512:
            LAUNCH_MASK_FUSED_LN_BWD_FAST_KERNEL(1, 512);
#else
          case 384:
...
#ifdef PADDLE_WITH_COREX
          case 512:
            LAUNCH_FUSED_LN_BWD_FAST_KERNEL(1, 512);
#else
          case 384:
...
#undef LAUNCH_FUSED_LN_BWD_FAST_KERNEL
    ...
    const int THREADS_PER_WARP_2 = WARP_SIZE;
...
#ifdef PADDLE_WITH_COREX
          case 512:
            LAUNCH_LN_BWD_BETA_GAMMMA_KERNEL(1, 512);
#else
          case 384:
...
__global__ void LayerNormBackwardGradientAll(
  ...
  __shared__ U shared_scale[WARP_SIZE];  // threadIdx.x / warpSize <= kMaxBlockDim /
                                  // warpSize <= 1024/32 = 32;
  __shared__ U shared_bias[WARP_SIZE];
...
#ifdef PADDLE_WITH_COREX
      if ((feature_size == 1024 || feature_size == 512 ||
#else
