...
__global__ void LayerNormForward(
  ...
  __shared__ U
      shared_mean[WARP_SIZE];  // threadIdx.x / warpSize <= kMaxBlockDim /
                               // warpSize <= 1024/32 = 32;
...
void ln_bwd_fast_kernel_driver(const phi::GPUContext &dev_ctx,
  ...
  if (cols == 1024 || cols == 384 || cols == 256) {
    ...
    const int VecSize =
        std::max(1, static_cast<int>(BYTES_PER_LDG / sizeof(T)));
#else
...
__global__ void LayerNormBackwardGradientAll(
  ...
  __shared__ U
      shared_scale[WARP_SIZE];  // threadIdx.x / warpSize <= kMaxBlockDim /
                                // warpSize <= 1024/32 = 32;
