...
inline int64_t max_shared_s_num(int64_t num_rows,
  ...
  if ((threads * vec_size) % dim == 0) {
    return std::min(num_rows, threads * vec_size / dim);
  } else {
    ...
    for (int64_t idx = 0; idx < num_rows * dim; idx += vec_size * threads) {
      ...
      int64_t si_end =
          std::min(num_rows * dim, idx + vec_size * threads - 1) / dim;
      max_res = std::max(max_res, (si_end - si_start + 1));
    }
    return std::min(num_rows, max_res);
  }
}
