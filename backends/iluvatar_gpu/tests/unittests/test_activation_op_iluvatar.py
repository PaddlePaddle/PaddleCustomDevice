    ...
    def test_errors(self):
        with static_guard():
            with paddle.static.program_guard(
                ...
                # The input dtype of sqrt op must be float16, float32, float32.
...
class TestExpm1API(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.place = []
        self.place.append(paddle.CustomPlace("iluvatar_gpu", 0))
    ...
    def test_errors(self):
        with static_guard():
            with paddle.static.program_guard(paddle.static.Program()):
                ...
                # The input dtype must be float16, float32, float32.
...
class TestTanh(TestActivation, TestParameter):
    ...
    def init_dtype(self):
        # TODO If dtype is float32, the output (Out) has diff at CPUPlace
...
class TestTanhshrinkAPI(unittest.TestCase):
    ...
    def setUp(self):
        np.random.seed(1024)
        self.x_np = np.random.uniform(10, 20, [10, 17]).astype(np.float32)
        self.place = paddle.CustomPlace("iluvatar_gpu", 0)
...
class TestHardtanhAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.place = (
            paddle.CustomPlace("iluvatar_gpu", 0)
...
class TestSoftshrinkAPI(unittest.TestCase):
    ...
    def setUp(self):
        self.threshold = 0.8
        np.random.seed(1024)
        self.x_np = np.random.uniform(0.25, 10, [10, 12]).astype(np.float32)
        self.place = (
            paddle.CustomPlace("iluvatar_gpu", 0)
            if paddle.is_compiled_with_cuda()
            else paddle.CPUPlace()
...
class TestGelu(TestActivation):
    def setUp(self):
        ...
        self.init_dtype()
        self.init_shape()
        # Todo: Under float32, only this accuracy is currently supported, for further processing
...
class TestRelu6API(unittest.TestCase):
    ...
    def setUp(self):
        np.random.seed(1024)
        self.x_np = np.random.uniform(-1, 10, [10, 12]).astype(np.float32)
...
class TestThresholdedReluAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.value = 5
        np.random.seed(1024)
        self.x_np = np.random.uniform(-20, 20, [10, 12]).astype(np.float32)
        self.x_np[np.abs(self.x_np) < 0.005] = 0.02
        self.place = (
            paddle.CustomPlace("iluvatar_gpu", 0)
            if paddle.is_compiled_with_cuda()
