...
class TestErrors(unittest.TestCase):
    ...
    def test_diagonalize_errors(self):
        a = np.arange(4 * 3 * 4 * 4).reshape(4, 3, 4, 4).astype("float32")
...
class TestEinsum(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.TEST_SAMPLES = {
            ...
            "y": np.random.rand(7).astype(np.float32),
            "A": np.random.rand(4, 5).astype(np.float32),
            "B": np.random.rand(2, 5).astype(np.float32),
            "C": np.random.rand(3, 7).astype(np.float32),
            "D": np.random.rand(3, 4, 5).astype(np.float32),
            "E": np.random.rand(3, 5, 2).astype(np.float32),
            "F": np.random.rand(2, 4, 5, 3).astype(np.float32),
            "G": np.random.rand(4, 2, 5).astype(np.float32),
            "H": np.random.rand(3, 2, 4).astype(np.float32),
            "I": np.random.rand(2, 2).astype(np.float32),
            "J": np.random.rand(1, 3, 5).astype(np.float32),
            "K": np.random.rand(1, 2, 3, 4).astype(np.float32),
        }
    ...
    def _get_place(self, force_to_use_cpu=False):
        if force_to_use_cpu:
            ...
            return paddle.CustomPlace("iluvatar_gpu", 0)
...
class TestNumpyTests(unittest.TestCase):
    ...
    def test_sums(self):
        ...
        for n in range(1, 17):
            a = np.arange(2 * 3 * n).reshape(2, 3, n).astype("float32")
            self.check_output("...i->...", a)

        for n in range(1, 17):
            a = np.arange(2 * n).reshape(2, n).astype("float32")
            self.check_output("i...->...", a)
        ...
        for n in range(1, 17):
            a = np.arange(3 * n).reshape(3, n).astype("float32")
            b = np.arange(2 * 3 * n).reshape(2, 3, n).astype("float32")
            self.check_output("..., ...", a, b)

        for n in range(1, 17):
            a = np.arange(2 * 3 * n).reshape(2, 3, n).astype("float32")
            b = np.arange(n).astype("float32")
            self.check_output("...i, ...i", a, b)

        for n in range(1, 11):
            a = np.arange(n * 3 * 2).reshape(n, 3, 2).astype("float32")
            b = np.arange(n).astype("float32")
            self.check_output("i..., i...", a, b)

        for n in range(1, 17):
            a = (np.arange(3) + 1).astype("float32")
            b = (np.arange(n) + 1).astype("float32")
            self.check_output("i,j", a, b)

        for n in range(1, 17):
            a = np.arange(4 * n).reshape(4, n).astype("float32")
            b = np.arange(n).astype("float32")
        ...
        for n in range(1, 17):
            a = np.arange(4 * n).reshape(4, n).astype("float32")
            b = np.arange(n * 6).reshape(n, 6).astype("float32")
            self.check_output("ij,jk", a, b)

        a = np.arange(12).reshape(3, 4).astype("float32")
        b = np.arange(20).reshape(4, 5).astype("float32")
        c = np.arange(30).reshape(5, 6).astype("float32")
        self.check_output("ij,jk,kl", a, b, c)

        a = np.arange(60).reshape(3, 4, 5).astype("float32")
        b = np.arange(24).reshape(4, 3, 2).astype("float32")
        ...
        p = np.ones((10, 2)).astype("float32")
        q = np.ones((1, 2)).astype("float32")
        self.check_output("ij,ij->j", p, q)

        x = np.array([2.0, 3.0]).astype("float32")
        y = np.array([4.0]).astype("float32")
        self.check_output("i, i", x, y)

        p = np.ones((1, 5)).astype("float32") / 2
        q = np.ones((5, 5)).astype("float32") / 2
        self.check_output("...ij,...jk->...ik", p, p.T)
        self.check_output("...ij,...jk->...ik", p, q)

        x = np.eye(2).astype("float32")
        y = np.ones(2).astype("float32")
        self.check_output("ji,i->", x, y)
        self.check_output("i,ij->", y, x)
        self.check_output("ij,i->", x, y)

    def test_large_nops(self):
        a = np.arange(4 * 3 * 1 * 4).reshape(4, 3, 1, 4).astype("float32")
    ...
    def test_static_graph(self):
        ...
        with base.program_guard(main, startup):
            ...
            d = paddle.static.data(name="d", shape=[None, None, 5], dtype="float32")
            e = paddle.static.data(name="e", shape=[None, 2, None], dtype="float32")
