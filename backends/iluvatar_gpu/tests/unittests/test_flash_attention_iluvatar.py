...
class TestFlashAttentionGQA(unittest.TestCase):
    ...
    def gen_unpadded_data(self, dtype):
        seq_len_q = np.random.randint(low=1, high=self.seq_len, size=[self.batch_size])
        seq_len_k = np.random.randint(low=1, high=self.seq_len, size=[self.batch_size])
        ...
        for i in range(self.batch_size):
            ...
            tmp_k = (
                paddle.randn(
                    [seq_len_k[i] * self.num_head * self.head_dim // self.num_group],
                    dtype=dtype,
                )
                / 1e2
            )
        ...
        q = paddle.concat(qs, axis=0).reshape([-1, self.num_head, self.head_dim])
    ...
    def calc_raw_attn(
        ...
        if use_unpadded:
            ...
            qk_mask = qk_mask.reshape([self.batch_size, 1, self.seq_len, self.seq_len])
