...
class TestlayernormOp(unittest.TestCase):
    def setUp(self):
        ...
        self.x_np = np.random.uniform(-0.05, 0.05, [batch, cols]).astype(np.float32)
        self.residual_np = np.random.uniform(-0.05, 0.05, [batch, cols]).astype(
            np.float32
        )
        self.bias_np = np.random.uniform(-0.05, 0.05, [cols]).astype(np.float32)
        self.norm_weight_np = np.random.uniform(-0.05, 0.05, [cols]).astype(np.float32)
        self.norm_bias_np = np.random.uniform(-0.05, 0.05, [cols]).astype(np.float32)
        self.epsilon = 1e-5
        self.residual_alpha = np.random.uniform(low=0.1, high=1.1, size=[1]).astype(
            np.float32
        )
...
class TestlayernormStaticOp(unittest.TestCase):
    def setUp(self):
        ...
        self.x_np = np.random.uniform(-0.05, 0.05, [self.batch, self.cols]).astype(
            np.float32
        )
        self.residual_np = np.random.uniform(
            -0.05, 0.05, [self.batch, self.cols]
        ).astype(np.float32)
        self.bias_np = np.random.uniform(-0.05, 0.05, [self.cols]).astype(np.float32)
        self.norm_weight_np = np.random.uniform(-0.05, 0.05, [self.cols]).astype(
            np.float32
        )
        self.norm_bias_np = np.random.uniform(-0.05, 0.05, [self.cols]).astype(
            np.float32
        )
        self.epsilon = 1e-5
        self.residual_alpha = np.random.uniform(low=0.1, high=1.1, size=[1]).astype(
    ...
    def test_residual_bias_add(self):
        (paddle_layernorm, paddle_naive_residual_out,) = self.check_residual_bias_add(
            self.x_np,
            self.residual_np,
            self.bias_np,
            "float16",
