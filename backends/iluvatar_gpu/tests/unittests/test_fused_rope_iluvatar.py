...
def paddle_fused_rotary_position_embedding(
    ...
    **kwargs,
    ...
    if position_ids is not None:
        ...
        sin_tensor = sin_tensor[position_ids].unsqueeze(2)  # [bs, seq_len, 1, dim]
        cos_tensor = cos_tensor[position_ids].unsqueeze(2)  # [bs, seq_len, 1, dim]
...
class TestFusedRotaryPositionEmbedding(unittest.TestCase):
    ...
    def get_inputs(self, seed, with_sin_cos, with_grads=False, rotate_half=False):
    ...
    def get_forward_backward(
        ...
        if test_time_major:
            ...
            if tensor_grad_outq is not None:
                tensor_grad_outq = paddle.transpose(tensor_grad_outq, perm=[1, 0])
            if tensor_grad_outk is not None:
                tensor_grad_outk = paddle.transpose(tensor_grad_outk, perm=[1, 0])
            if tensor_grad_outv is not None:
                tensor_grad_outv = paddle.transpose(tensor_grad_outv, perm=[1, 0])
        ...
        bw = list(filter(lambda x: x is not None, [tensor_q, tensor_k, tensor_v]))
    ...
    def test_static(self):
        ...
        with paddle.static.program_guard(main, startup):
            q = (
                ...
                else paddle.static.data(name="q", shape=self.shape_q, dtype=self.dtype)
            )

            k = (
                None
                if self.shape_k is None
                else paddle.static.data(name="k", shape=self.shape_k, dtype=self.dtype)
            )

            v = (
                None
                if self.shape_v is None
                else paddle.static.data(name="v", shape=self.shape_v, dtype=self.dtype)
            )
    ...
    def test_static_time_major(self):
        ...
        with paddle.static.program_guard(main, startup):
            q = (
                None
                if shape_q is None
                else paddle.static.data(name="q", shape=shape_q, dtype=self.dtype)
            )

            k = (
                None
                if shape_k is None
                else paddle.static.data(name="k", shape=shape_k, dtype=self.dtype)
            )

            v = (
                None
                if shape_v is None
                else paddle.static.data(name="v", shape=shape_v, dtype=self.dtype)
            )
