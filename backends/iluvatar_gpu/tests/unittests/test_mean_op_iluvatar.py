...
class TestMeanOp_float32ZeroSize(OpTest):
    def setUp(self):
        self.op_type = "mean"
        self.python_api = paddle.mean
        self.dtype = np.float32
...
class TestMeanOp_float32ZeroSize3D(TestMeanOp_float32ZeroSize):
    def setUp(self):
        self.op_type = 'mean'
        self.python_api = paddle.mean
        self.dtype = np.float32
...
class TestMeanOp_RealValuedNanInput(OpTest):
    def setUp(self):
        self.op_type = "mean"
        self.python_api = paddle.mean
        self.inputs = {'X': np.array([1, 2, 3, np.nan]).astype("float32")}
        self.outputs = {'Out': np.mean(self.inputs["X"])}
