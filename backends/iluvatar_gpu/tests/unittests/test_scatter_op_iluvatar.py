...
class TestScatterOp(OpTest):
    def setUp(self):
        ...
        random_negative_mask = (np.random.rand(index_np.shape[0]) > 0.5).astype("bool")
...
class TestScatterBF16Op(TestScatterOp):
    ...
    def test_check_output(self):
        place = core.CustomPlace("iluvatar_gpu", 0)
...
class TestScatterNegativeAxis(OpTest):
    ...
    def test_check_output(self):
        ...
        places.append(paddle.CustomPlace("iluvatar_gpu", 0))
        for place in places:
...
class TestScatterAPI(unittest.TestCase):
    def setUp(self):
        self.places = [paddle.CustomPlace("iluvatar_gpu", 0)]
        self.executed_api()
        ...
        def test_static_graph():
            with paddle.static.program_guard(
                ...
                with paddle.static.scope_guard(scope):
                    ...
                    fetch = [out_t]
                    gpu_exe = paddle.static.Executor(
...
class TestScatterOpFp16(OpTest):
    ...
    def test_scatter_fp16(self):
        paddle.disable_static(place=paddle.CustomPlace("iluvatar_gpu", 0))
