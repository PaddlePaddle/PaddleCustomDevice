...
class TestSwiGLUDygraph(unittest.TestCase):
    ...
    def check_dygraph(self, shape):
        metas = [('cpu', paddle.float32), ('cpu', paddle.float32)]
        metas.append(('iluvatar_gpu', paddle.float32))
        metas.append(('iluvatar_gpu', paddle.float16))
        metas.append(('iluvatar_gpu', paddle.bfloat16))
...
class TestSwigluOp(OpTest):
    ...
    def setUp(self):
        ...
        y = np.random.uniform(-1, 1, self.x_shape).astype("float32")
        out_grad = np.random.uniform(-1, 1, self.x_shape).astype("float32")
...
class TestSwigluOp2(TestSwigluOp):
    def setUp(self):
        ...
        x = tmp_inputs[0]
        y = tmp_inputs[1]
        out_grad = np.random.uniform(-1, 1, x.shape).astype("float32")
        res = swiglu(x, y, out_grad)
        self.inputs = {'x': x, 'y': y}
        self.outputs = {'out': res[0].numpy()}
        self.placements = {
