...
void ReduceKernel(const phi::Context& dev_ctx,
  ...
  show_kernel(
      kernel_name << "-Sycl type=" << dnn_support::type2String<T>::name()
                  << ", reduce_all=" << reduce_all << ", x_dims=" << x_dims
                  << ", dims=" << dims << ", keep_dim=" << keep_dim
  ...
  if (x_dims == reduce_dims) {
    ...
    show_debug(kernel_name << " -> memcpy(to=" << std::hex << out_data
                           << ", from=" << x_data << ", size=" << std::dec
                           << x.memory_size() << ")");
    ...
    auto md_out = dnnl::memory::desc(reduce_dims,
