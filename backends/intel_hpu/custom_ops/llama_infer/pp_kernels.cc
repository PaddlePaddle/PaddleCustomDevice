...
std::vector<paddle::Tensor> TokenPenaltyMultiScores(
  ...
  if (recipe == nullptr) {
    ...
    synTensor logits_no_eos = op.createTensorFromCT(
        ct, LOGITS, "logits_no_eos", true, section_shared);
    op.AddMinLengthLogitsNode(
        logits_in, cur_len_i32, min_len_i32, eos_i32, logits_no_eos);
