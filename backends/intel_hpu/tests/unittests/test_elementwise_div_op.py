...
class TestElementwiseDivOp_broadcast(TestElementwiseDivOp):
    def init_input(self):
        self.x = np.random.uniform(-1, 1, [13, 17]).astype(self.dtype)
        self.y = np.random.uniform(-1, 1, [17]).astype(self.dtype)


class TestFP16ElementwiseDivOp(TestElementwiseDivOp):
    def init_dtype(self):
        self.dtype = np.float16


class TestFP16ElementwiseDivOp_1(TestElementwiseDivOp_broadcast):
    def init_dtype(self):
        self.dtype = np.float16


class TestFP8ElementwiseDivOp(unittest.TestCase):
    def setUp(self):
        self.place = paddle.CustomPlace("intel_hpu", int(intel_hpus_module_id))
        self.dtype = np.float16
        self.prepare_input()
    ...
    def prepare_input(self):
        np.random.seed(1024)
        self.x0 = np.random.random((8, 8)).astype(self.dtype)
        self.x1 = np.random.random((8, 8)).astype(self.dtype)
    ...
    def test_check_output(self):
        ...
        out = paddle.divide(x0_fp8, x1_fp8).astype("float16")
