...
class TestElementwiseSubOp_broadcast(TestElementwiseSubOp):
    def init_input(self):
        self.x = np.random.uniform(-1, 1, [13, 17]).astype(self.dtype)
...
class TestFP16ElementwiseSubOp_1(TestElementwiseSubOp_broadcast):
    def init_dtype(self):
        self.dtype = np.float16


class TestFP8ElementwiseSubOp(unittest.TestCase):
    def setUp(self):
        self.place = paddle.CustomPlace("intel_hpu", int(intel_hpus_module_id))
        self.dtype = np.float16
        self.prepare_input()
    ...
    def test_check_output(self):
        ...
        out = paddle.subtract(x0_fp8, x1_fp8).astype("float16")
