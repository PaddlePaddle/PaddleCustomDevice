...
@ddt
class TestHPUWhereCommon(TestWhere):
    @data(*WHERE_CASE)
    @unpack
    def test_check_output(self, x_shape, y_shape, dtype):
        ...
        self.dtype = dtype
        self.check_output_hpu_with_cpu(self.forward)


@ddt
class TestHPUWhereF16(TestWhere):
    @data(*WHERE_F16_CASE)
    @unpack
    def test_check_output(self, x_shape, y_shape, dtype):
        self.x_shape = x_shape
        self.y_shape = y_shape
        self.dtype = dtype
        self.check_output_hpu_with_customized(self.forward, self.where_cast)


@ddt
class TestHPUWhereNoneCommon(TestWhere):
    ...
    def test_check_output(self, x_shape, y_shape, dtype):
        ...
        self.y_shape = y_shape
        self.dtype = dtype
        self.check_output_hpu_with_cpu(self.where_none_forward)


@ddt
class TestHPUWhereOtherCommon(TestWhere):
    def prepare_data(self):
        self.data_x = self.generate_data(self.x_shape, self.dtype)
        self.data_y = self.generate_data(self.y_shape, self.dtype)
    ...
    def test_check_output(self, x_shape, y_shape, dtype):
        ...
        self.y_shape = y_shape
        self.dtype = dtype
        self.check_output_hpu_with_cpu(self.where_other_forward)
