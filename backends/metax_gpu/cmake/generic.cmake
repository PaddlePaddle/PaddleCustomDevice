...
# Reserved. Copyright (c) 2016 PaddlePaddle Authors. All Rights Reserved.
#
...
# generic.cmake defines CMakes functions that look like Bazel's building rules
# (https://bazel.build/).
#
# -------------------------------------------
# C++        CUDA C++       Go
# -------------------------------------------
# cc_library    nv_library   go_library cc_test       nv_test      go_test
# -------------------------------------------
#
# To build a static library example.a from example.cc using the system compiler
# (like GCC):
#
...
#
# To build a library using Nvidia's NVCC from .cu file(s), use the nv_ prefixed
# version:
#
...
#
# To build a unit test binary, which is an executable binary with GoogleTest
# linked:
#
...
#
# It is pretty often that executable and test binaries depend on pre-defined
# external libraries like glog and gflags defined in /cmake/external/*.cmake:
#
...
# find all fluid modules is used for paddle fluid static library for building
# inference libs
...
# NOTE(Aurelius84): NOT_INFER_MODULES is used to tag and not considered as DEPS
# for inference libs.
...
# find all phi modules is used for paddle static library for building inference
# libs
...
# find all third_party modules is used for paddle static library for reduce the
# dependency when building the inference libs.
...
function(merge_static_libs TARGET_NAME)
  ...
  # To produce a library we need at least one source file. It is created by
  # add_custom_command below and will helps also help to track dependencies.
  set(target_SRCS ${CMAKE_CURRENT_BINARY_DIR}/${TARGET_NAME}_dummy.c)

  # Make the generated dummy source file depended on all static input libs. If
  # input lib changes,the source file is touched which causes the desired effect
  # (relink).
  add_custom_command(
    OUTPUT ${target_SRCS}
  ...
  # Windows do not support gcc/nvcc combined compiling. Use msvc 'lib.exe' to
  # merge libs.
  if(WIN32)
    foreach(lib ${libs})
      set(libfiles ${libfiles} $<TARGET_FILE:${lib}>)
    endforeach()
    # msvc compiler will put library in directory of "/Release/xxxlib" by
    # default
    add_custom_command(
      TARGET ${TARGET_NAME}
      POST_BUILD
...
function(check_coverage_opt TARGET_NAME SRCS)
  if(WITH_COVERAGE AND WITH_INCREMENTAL_COVERAGE)
    # if pybind.cc add '-g -O0 -fprofile-arcs -ftest-coverage' only, some
    # testcase will fail.
...
function(cc_test TARGET_NAME)
  if(WITH_TESTING)
    ...
    if(WIN32)
      ...
      # explicitly by __declspec(dllexport), however, there are several symbols
      # not exported, and link error occurs. so, the tests are not built against
      # dynamic libraries now.
      cc_test_old(
        ${TARGET_NAME}
        SRCS
        ${cc_test_SRCS}
      ...
      # message("cc_test_SRCS ${cc_test_SRCS}") message("cc_test_ARGS
      # ${cc_test_ARGS}")
...
function(cc_test_old TARGET_NAME)
  if(WITH_TESTING)
    ...
    # we do not test hcom op, because it need complex configuration with more
    # than one machine
...
function(nv_library TARGET_NAME)
  if(WITH_GPU)
    ...
    cmake_parse_arguments(nv_library "${options}" "${oneValueArgs}"
                          "${multiValueArgs}" ${ARGN})
    if(nv_library_SRCS)
      # Attention: 1. cuda_add_library is deprecated after cmake v3.10, use
      # add_library for CUDA please. 2. cuda_add_library does not support
      # ccache. Reference:
...
function(nv_test TARGET_NAME)
  if(WITH_GPU AND WITH_TESTING)
    ...
    cmake_parse_arguments(nv_test "${options}" "${oneValueArgs}"
                          "${multiValueArgs}" ${ARGN})
    # Attention: 1. cuda_add_executable is deprecated after cmake v3.10, use
    # cuda_add_executable for CUDA please. 2. cuda_add_executable does not
    # support ccache. Reference:
    # https://cmake.org/cmake/help/v3.10/module/FindCUDA.html
...
function(hip_binary TARGET_NAME)
  if(WITH_ROCM)
    ...
    cmake_parse_arguments(hip_binary "${options}" "${oneValueArgs}"
                          "${multiValueArgs}" ${ARGN})
    # FindHIP.cmake defined hip_add_executable, HIP_SOURCE_PROPERTY_FORMAT is
    # required for .cc files
...
function(go_library TARGET_NAME)
  ...
  # This custom command will always run since it depends on a not existing file.
  add_custom_command(OUTPUT dummy_rebuild_${TARGET_NAME} COMMAND cmake -E touch
                                                                 ${dummyfile})
  # Create a custom target that depends on the custom command output file, so
  # the custom command can be referenced as a dependency by `add_dependencies`.
  ...
  # The "source file" of the library is `${dummyfile}` which never change, so
  # the target will never rebuild. Make the target depends on the custom command
  # that touches the library "source file", so rebuild will always happen.
...
# Usage: paddle_protobuf_generate_cpp(<proto_srcs> <proto_hdrs> <proto_files>)
...
# grpc_library generate grpc code using grpc_cpp_plugin and protoc then build
# the generated protobuf code and grpc code with your implementation source
# codes together. Use SRCS argument for your implementation source files and
# PROTO argument for your .proto files.
#
# Usage: grpc_library(my_target SRCS my_client.cc PROTO my_target.proto DEPS
# my_dep)
...
function(grpc_library TARGET_NAME)
  ...
  # FIXME(typhoonzero): grpc generated code do not generate virtual-dtor, mark
  # it as compiler warnings instead of error. Should try remove the warnings
  # also.
...
function(brpc_library TARGET_NAME)
  ...
  cc_library("${TARGET_NAME}" SRCS "${brpc_library_SRCS}" DEPS
             "${TARGET_NAME}_proto" "${brpc_library_DEPS}")
endfunction()
...
function(copy_if_different src_file dst_file)
  ...
  # this is a dummy target for custom command, should always be run firstly to
  # update ${dst_file}
  add_custom_target(
...
# create a dummy source file, then create a static library. LIB_NAME should be
# the static lib name. FILE_PATH should be the dummy source file path. GENERATOR
# should be the file name invoke this function. CONTENT should be some helpful
# info. example: generate_dummy_static_lib(mylib FILE_PATH /path/to/dummy.c
# GENERATOR mylib.cmake CONTENT "helpful info")
...
function(math_library TARGET)
  # math_library is a function to create math library. The interface is the same
  # as cc_library. But it handle split GPU/CPU code and link some common
  # library.
  set(cc_srcs)
  set(cu_srcs)
  set(hip_srcs)
  ...
  elseif(${cc_srcs_len} GREATER 0)
    cc_library(${TARGET} SRCS ${cc_srcs} DEPS ${math_library_DEPS}
               ${math_common_deps})
  endif()
endfunction()
