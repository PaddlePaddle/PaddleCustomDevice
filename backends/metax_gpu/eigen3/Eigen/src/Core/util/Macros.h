...
// NOTE(xbei): both __NVCC__ and __MACACC__ will be defined when using cuda wrapper tools chain.
//   when comipled by nvcc, only __NVCC__ will be defined.
// #if defined(__NVCC__)
...
// NOTE(xbei): when compile with nvcc, only __CUDACC__ would be defined.
//   When using cuda wrapper, both __CUDACC__ and __MACACC__ would be defined.
#if defined(__CUDACC__) && !defined(EIGEN_NO_CUDA) && !defined(__MACACC__)
  // Means the compiler is either nvcc or clang with CUDA enabled
  #define EIGEN_CUDACC __CUDACC__
#endif

// NOTE(xbei): when compile with nvcc, only __CUDA_ARCH__ would be defined.
//   When using cuda wrapper, both __CUDA_ARCH__ and __MACA_ARCH__ would be defined.
...
#if EIGEN_MAX_CPP_VER>=11 && (EIGEN_COMP_CXXVER >= 11) \
  && (!(defined(__NVCC__) && !defined(__MACACC__)) || !EIGEN_ARCH_ARM_OR_ARM64 || (EIGEN_COMP_NVCC >= 80000) )
...
// Does the compiler fully support const expressions? (as in c++14)
  ...
  #if defined(EIGEN_CUDACC) || defined(EIGEN_MACACC)
...
// Disable some features when compiling with GPU compilers (NVCC/clang-cuda/SYCL/HIPCC/MXCC)
#if defined(EIGEN_CUDACC) || defined(SYCL_DEVICE_ONLY) || defined(EIGEN_HIPCC) || defined(EIGEN_MACACC)
...
#if (defined(EIGEN_CUDA_ARCH) && defined(__NVCC__) && !defined(__MACACC__)) || defined(EIGEN_HIP_DEVICE_COMPILE) || defined(EIGEN_MACA_ARCH)
