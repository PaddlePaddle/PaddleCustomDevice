...
namespace internal {
// NOTE(xbei): mxcc compiling failed, note: candidate template ignored: target attributes do not match
#if defined(EIGEN_MACACC)
  template<> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE double cast(const AnnoyingScalar& x) { return double(*x.v); }
  template<> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE float  cast(const AnnoyingScalar& x) { return *x.v; }
#else
