...
EIGEN_DEVICE_FUNC bits(const T& x) {
...
EIGEN_DEVICE_FUNC T apply_bit_op(Bits a, Bits b, Func f) {
  Array<unsigned char,sizeof(T),1> data;
  T res;
  for(Index i = 0; i < data.size(); ++i)
    data[i] = f(a[i], b[i]);
  // Note: The reinterpret_cast works around GCC's class-memaccess warnings:
  memcpy(reinterpret_cast<unsigned char*>(&res), data.data(), sizeof(T));
  return res;
}

#define EIGEN_TEST_MAKE_BITWISE2(OP,FUNC,T)                               \
  template<> EIGEN_DEVICE_FUNC T EIGEN_CAT(p,OP)(const T& a,const T& b) { \
...
#define EIGEN_TEST_MAKE_BITWISE(OP,FUNC)                  \
  ...
  EIGEN_TEST_MAKE_BITWISE2(OP,FUNC,Eigen::bfloat16)       \
...
// NOTE(xbei): std::bit_xor(), std::bit_and(), std::bit_or() are __host__ routines, which
//   can NOT be called in __host__ __device__ function.
//   when test on GPU, use builtin routines bit_or(), bit_and(), bit_or() instead.
EIGEN_TEST_MAKE_BITWISE(xor,bit_xor<unsigned char>())
EIGEN_TEST_MAKE_BITWISE(and,bit_and<unsigned char>())
EIGEN_TEST_MAKE_BITWISE(or, bit_or<unsigned char>())
#else
...
#endif
struct bit_andnot{
  template<typename T> EIGEN_DEVICE_FUNC T
