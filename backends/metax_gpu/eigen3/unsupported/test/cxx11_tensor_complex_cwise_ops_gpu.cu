...
template<typename T>
void test_gpu_complex_cwise_ops() {
  ...
  for (int op = Add; op < NbOps; op++) {
    ...
    assert(gpuMemcpyAsync(actual.data(), d_out, complex_bytes, gpuMemcpyDeviceToHost,
...
{
  CALL_SUBTEST(test_gpu_complex_cwise_ops<float>());
  CALL_SUBTEST(test_gpu_complex_cwise_ops<double>());
}
