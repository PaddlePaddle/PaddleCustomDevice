...
void test_gpu_nullary() {
  ...
  assert(gpuMemcpyAsync(new1.data(), d_in1, complex_bytes, gpuMemcpyDeviceToHost,
                         gpu_device.stream()) == gpuSuccess);
  assert(gpuMemcpyAsync(new2.data(), d_out2, float_bytes, gpuMemcpyDeviceToHost,
                         gpu_device.stream()) == gpuSuccess);
...
static void test_gpu_sum_reductions() {
...
static void test_gpu_mean_reductions() {
...
static void test_gpu_product_reductions() {
...
{
  CALL_SUBTEST(test_gpu_nullary());
  CALL_SUBTEST(test_gpu_sum_reductions());
  CALL_SUBTEST(test_gpu_mean_reductions());
  CALL_SUBTEST(test_gpu_product_reductions());
}
