...
void test_gpu_numext() {
  ...
  for (int i = 0; i < num_elem; ++i) {
    std::cout << "Checking numext " << i << ": full prec = " << full_prec(i) << " vs half prec = " << half_prec(i) << std::endl;
...
void test_gpu_conversion() {
  ...
  for (int i = 0; i < num_elem; ++i) {
    std::cout << "Checking conversion " << i << ": initial = " << initial(i) << " vs final = " << final(i) << std::endl;
...
void test_gpu_unary() {
  ...
  for (int i = 0; i < num_elem; ++i) {
    std::cout << "Checking unary " << i << ": full prec = " << full_prec(i) << " vs half prec = " << half_prec(i) << std::endl;
...
void test_gpu_contractions() {
  ...
  Eigen::half* d_cast_float1 = (Eigen::half*)gpu_device.allocate(num_elem * sizeof(Eigen::half));
  Eigen::half* d_cast_float2 = (Eigen::half*)gpu_device.allocate(num_elem * sizeof(Eigen::half));
  ...
  Eigen::TensorMap<Eigen::Tensor<Eigen::half, 2>, Eigen::Aligned> gpu_cast_float1(
      d_cast_float1, rows, cols);
  Eigen::TensorMap<Eigen::Tensor<Eigen::half, 2>, Eigen::Aligned> gpu_cast_float2(
      d_cast_float2, rows, cols);
  ...
  gpu_cast_float1.device(gpu_device) = gpu_float1.cast<Eigen::half>();
  gpu_cast_float2.device(gpu_device) = gpu_float2.cast<Eigen::half>();
  ...
  gpu_device.synchronize();
  Tensor<float, 2> float1(rows, cols);
  Tensor<float, 2> float2(rows, cols);
  gpu_device.memcpyDeviceToHost(float1.data(), d_float1, num_elem*sizeof(float));
  gpu_device.memcpyDeviceToHost(float2.data(), d_float2, num_elem*sizeof(float));
  gpu_device.synchronize();
  Tensor<Eigen::half, 2> h_float1(rows, cols);
  Tensor<Eigen::half, 2> h_float2(rows, cols);
  gpu_device.memcpyDeviceToHost(h_float1.data(), d_cast_float1, num_elem*sizeof(Eigen::half));
  gpu_device.memcpyDeviceToHost(h_float2.data(), d_cast_float2, num_elem*sizeof(Eigen::half));
  gpu_device.synchronize();
  ...
  std::cout << "h_float1: " << std::endl;
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      std::cout << h_float1(i, j) << "\t";
    }
    std::cout << std::endl;
  }

  std::cout << "h_float2: " << std::endl;
  for (int i = 0; i < rows; ++i) {
    for (int j = 0; j < cols; ++j) {
      std::cout << h_float2(i, j) << "\t";
    }
    std::cout << std::endl;
  }
...
void test_gpu_reductions(int size1, int size2, int redux) {
  ...
  for (int i = 0; i < result_size; ++i) {
    std::cout << "Checking reductions " << i << ": full prec = " << full_prec(i) << " vs half prec = " << half_prec(i) << std::endl;
    VERIFY_IS_APPROX(full_prec(i), half_prec(i));
  }
...
void test_gpu_forced_evals() {
  ...
  for (int i = 0; i < num_elem; ++i) {
    std::cout << "Checking forced eval " << i << ": full prec = " << full_prec(i) << " vs half prec1 = " << half_prec1(i) << " vs half prec2 = " << half_prec2(i) << std::endl;
