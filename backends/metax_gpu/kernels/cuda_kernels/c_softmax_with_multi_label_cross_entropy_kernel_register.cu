...
// #include "kernels/funcs/collective_helper.h"
...
//                      "be less than [%ld] or equal to [%ld], but received
//                      [%ld]", static_cast<int64_t>(D * nranks),
//                      static_cast<int64_t>(ignore_index),
//                      static_cast<int64_t>(real_label));

//       if (real_label >= start_index && real_label < end_index) {
//         predicted_logits[i * C + j] = logit[i * D + real_label -
//         start_index];
//       }
//     }
//   }
...
//                                               const DenseTensor& label,
//                                               const DenseTensor&
//                                               smooth_weight, int64_t
//                                               ignore_index, bool
//                                               sum_multi_label_loss, int rank,
//                                               int nranks,
//                                               DenseTensor* softmax,
//                                               DenseTensor* loss) {
...
//     dev_ctx.template Alloc<T>(&predicted_logits);
//     comm_ctx->AllReduce(&predicted_logits, predicted_logits, ncclSum,
//     stream);
...
//         dev_ctx, softmax_2d, {-1}, softmax_2d.dtype(), true,
//         &sum_exp_logits);
...
// PD_REGISTER_PLUGIN_KERNEL(c_softmax_with_multi_label_cross_entropy,
//                    metax_gpu,
//                    ALL_LAYOUT,
