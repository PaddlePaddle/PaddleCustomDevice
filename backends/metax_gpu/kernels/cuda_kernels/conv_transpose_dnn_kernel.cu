...
//       &paddings_, &dilations_, padding_algorithm, x_data_dims, strides,
//       ksize);
...
//   workspace_size =
//       std::max(workspace_size, search::GetWorkspaceSize(args,
//       bwd_result.algo));
// #endif

//   // ------------------- cudnn conv transpose forward ---------------------
//   int x_offset = transformed_x.numel() / transformed_x.dims()[0] / groups;
//   int out_offset = transformed_out.numel() / transformed_out.dims()[0] /
//   groups; int filter_offset = filter.numel() / groups; ScalingParamType<T>
//   alpha = 1.0f; ScalingParamType<T> beta = 0.0f;
//   // auto workspace_handle = dev_ctx.cudnn_workspace_handle();

//   auto workspace_handle = GetDnnWorkspaceHandle(dev_ctx.stream(),
//   &dev_ctx.GetAllocator(), dev_ctx.GetPlace());
...
//     } else if (strides.size() == 3U) {
//       out_transpose = Transpose<T, Context>(dev_ctx, out_nchw, {0, 2, 3, 4,
//       1});
//     }
//     *out = out_transpose;
//   }
