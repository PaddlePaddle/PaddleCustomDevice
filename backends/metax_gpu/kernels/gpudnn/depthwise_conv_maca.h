...
WarpReduceSum(T val,                                // NOLINT
              unsigned long long lane_mask) {       // NOLINT
  for (int mask = HALF_WARP; mask > 0; mask >>= 1)  // NOLINT
...
template <typename T>
__forceinline__ __device__ T
BlockReduceSum(T val, unsigned long long mask = FINAL_MASK) {  // NOLINT
...
#define ARG_DEFINE_KernelDepthwiseConvInputGradS                               \
  const T *const input_data, const T *const output_grad_data,                  \
      ...
      T *const input_grad_data, FastDivMod input_width_r,                      \
      FastDivMod input_height_r, FastDivMod input_channels_r
...
__device__ __inline__ void KernelDepthwiseConvInputGradCFilterNCHWS(
  ...
  for (int c_i = 0; c_i < filter_multiplier; c_i++) {
    ...
    if (s_back < 4) {
      ...
      s_grad[tid * 4 - s_back + 1] =
          output_grad_data[output_grad_offset - s_back + 1];
      s_grad[tid * 4 - s_back + 2] =
          output_grad_data[output_grad_offset - s_back + 2];
      s_grad[tid * 4 - s_back + 3] =
          output_grad_data[output_grad_offset - s_back + 3];
    }
    __syncthreads();
...
__device__ __inline__ void KernelDepthwiseConvFilterGradNCHW(
  ...
  if (loop_batch) {
    ...
    if (!fuse_relu_before_conv) f_grad += out_grad_tmp * in_data_tmp;
  } else {
    T out_grad_tmp(0), in_data_tmp(0);
...
#define check_case(c_filter_multiplier, c_stride, c_filter)                  \
  static_assert(c_stride <= 2, "Something bad may happen when stride > 2."); \
  if (c_filter_multiplier == 0 ||                                            \
      filter_multiplier == c_filter_multiplier &&                            \
          stride_height == stride_width && stride_height == c_stride &&      \
          (ksize_height == ksize_width && ksize_height == c_filter ||        \
