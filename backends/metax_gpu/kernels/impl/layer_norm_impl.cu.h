...
static __forceinline__ __device__ U WarpReduceSum(U val) {
  // unsigned mask = 0u;
  unsigned long long mask = 0ull;  // NOLINT
...
#pragma unroll
    for (int mask = BDIMX / 2; mask > 0; mask /= 2) {
      ...
      // WARP_SHFL_XOR(sum_loss, mask);
      sum_loss1 +=
          __shfl_xor_sync(0xffffffffffffffffull, sum_loss1, mask, warpSize);
      sum_loss2 +=
          __shfl_xor_sync(0xffffffffffffffffull, sum_loss2, mask, warpSize);
#endif
    }
    // inter-warp reductions
    if (BDIMY > 1) {
...
#else
  for (int64_t bid = bid = blockIdx.x * blockDim.z + threadIdx.z; bid < n1;
       bid += gridDim.x * blockDim.z) {
#endif
    U sum_loss1 = U(0);
    U sum_loss2 = U(0);
