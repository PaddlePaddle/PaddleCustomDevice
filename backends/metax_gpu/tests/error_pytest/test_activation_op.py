        ...
        with dynamic_guard():
            for device in devices:
                if device == "cpu" or (
                    ...
                    x_paddle = paddle.to_tensor(x_np, dtype=self.dtype, place=device)
...
class TestRelu_NanInput(TestActivation):
    ...
    def test_check_output(self):
        self.assertTrue(paddle.isnan(self.outputs_paddle).cast("int32").sum() > 0)
...
class TestHardSwish(TestActivation):
    ...
    def test_check_grad(self):
        self.check_grad(
            ...
            "Out",
            check_prim=(
                True if self.dtype not in [np.complex64, np.complex128] else False
            ),
...
class TestSoftRelu(TestActivation):
    ...
    def test_check_output(self):
        self.check_output(check_dygraph=False, check_pir_onednn=self.check_pir_onednn)
...
class TestLog_Complex64(TestLog):
    ...
    def test_grad_grad(self):
        ...
        y = paddle.log(x)
        dx = paddle.grad(outputs=[y], inputs=[x], create_graph=True, retain_graph=True)[
            0
        ]
...
class TestPow_API(TestActivation):
    def test_api(self):
        with static_guard():
            ...
            np.testing.assert_allclose(res_1, np.power(input, 2), rtol=1e-5, atol=1e-8)
            np.testing.assert_allclose(res_2, np.power(input, 3), rtol=1e-5, atol=1e-8)
            np.testing.assert_allclose(res_6, np.power(input, 3), rtol=1e-5, atol=1e-8)
...
create_test_act_fp16_class(TestLeakyRelu_ZeroDim, check_prim=True, check_prim_pir=True)
create_test_act_fp16_class(
    TestRsqrt,
    check_prim=True,
    enable_cinn=True,
...
create_test_act_bf16_class(TestExpFp32_Prim, check_prim=True, check_prim_pir=True)
...
create_test_act_bf16_class(TestLeakyReluAlpha1, check_prim=True, check_prim_pir=True)
create_test_act_bf16_class(TestLeakyReluAlpha2, check_prim=True, check_prim_pir=True)
create_test_act_bf16_class(TestLeakyReluAlpha3, check_prim=True, check_prim_pir=True)
create_test_act_bf16_class(TestLeakyRelu_ZeroDim, check_prim=True, check_prim_pir=True)
create_test_act_bf16_class(
