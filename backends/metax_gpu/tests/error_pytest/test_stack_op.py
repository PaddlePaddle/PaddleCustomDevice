...
class TestPrimStackGrad(unittest.TestCase):
    ...
    def test_stack_double_grad(self):
        ...
        ggrads_expected = paddle.grad(grads_expected, self.x[1], create_graph=False)[0]
    ...
    def test_stack_triple_grad(self):
        ...
        zz = paddle.tanh(self.x[1])
        grads_expected = paddle.grad(zz, self.x[1], create_graph=True)
        ggrads_expected = paddle.grad(grads_expected, self.x[1], create_graph=True)
        gggrads_expected = paddle.grad(ggrads_expected, self.x[1], create_graph=True)[0]
