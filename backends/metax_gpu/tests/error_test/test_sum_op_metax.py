...
class TestSelectedRowsSumOp(unittest.TestCase):
    ...
    def check_with_place(self, place, inplace):
        self.check_input_and_output(core.Scope(), place, inplace, True, True, True)
        self.check_input_and_output(core.Scope(), place, inplace, False, True, True)
        self.check_input_and_output(core.Scope(), place, inplace, False, False, True)
        self.check_input_and_output(core.Scope(), place, inplace, False, False, False)
...
class TestSelectedRowsSumBF16Op(TestSelectedRowsSumOp):
    ...
    def check_input_and_output(
        ...
        if has_data_w_num > 0:
            ...
            ref_fp32 = (
                convert_uint16_to_float(self._get_array(self.rows, self.row_numel))
                * has_data_w_num
            )
    ...
    def test_dygraph(self):
        with dygraph_guard():
            ...
            for dtype_input in self.test_dtypes:
                paddle_result = paddle.sum(x_paddle, axis=self.axis, dtype=dtype_input)
...
class TestSumOpDtype(unittest.TestCase):
    ...
    def test_dygraph(self):
        with dygraph_guard():
            ...
            paddle_result = x_paddle.sum(axis=self.axis, dtype=self.output_dtype)
