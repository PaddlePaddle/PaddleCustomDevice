...
def create_test_AxisTensor(parent):
    class TestConcatAxisTensor(parent):
        ...
        def test_check_grad(self):
            if parent.__name__ == "TestConcatOp4" or parent.__name__ == "TestConcatOp3":
                return
            if self.dtype == np.uint16:
                place = core.CUDAPlace(0)
...
class TestConcatOpError(unittest.TestCase):
    def test_errors(self):
        ...
        with paddle.base.program_guard(paddle.base.Program(), paddle.base.Program()):
            ...
            x2 = base.create_lod_tensor(np.array([[-1]]), [[1]], base.CPUPlace())
            x3 = base.create_lod_tensor(np.array([[-1]]), [[1]], base.CPUPlace())
...
class TestConcatAPI(unittest.TestCase):
    def test_base_api(self):
        ...
        with paddle.base.program_guard(paddle.base.Program()):
            x_1 = paddle.static.data(shape=[None, 1, 4, 5], dtype="int32", name="x_1")
...
class TestConcatOpErrorWithPir(unittest.TestCase):
    ...
    def test_empty_inputs_static(self):
        with (
            IrGuard(),
            paddle.base.program_guard(paddle.base.Program(), paddle.base.Program()),
        ):
            with self.assertRaisesRegex(ValueError, "but got empty list"):
