...
def run_static(x_np, y_np, op_str, use_metax_gpu=False, binary_op=True):
    paddle.enable_static()
...
def run_dygraph(x_np, y_np, op_str, use_metax_gpu=False, binary_op=True):
    place = paddle.CustomPlace("metax_gpu", 0)
...
def test(unit_test, use_metax_gpu=False, test_error=False):
    for op_data in TEST_META_OP_DATA:
        meta_data = dict(op_data)
        meta_data["use_metax_gpu"] = use_metax_gpu
...
def test_type_error(unit_test, use_metax_gpu, type_str_map):
...
class Testmetax_gpu(unittest.TestCase):
    def test(self):
        test(self, True)
