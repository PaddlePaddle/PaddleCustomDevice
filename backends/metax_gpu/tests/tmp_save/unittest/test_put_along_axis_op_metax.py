...
class TestPutAlongAxisOpCase2(TestPutAlongAxisOp):
    ...
    def init_data(self):
        ...
        self.value = np.arange(1, 126).reshape((5, 5, 5)).astype(self.value_type)
        self.index_type = "int64"
...
class TestPutAlongAxisOpAdd(TestPutAlongAxisOp):
    ...
    def init_data(self):
        ...
        self.x_shape = (10, 10, 10)
        self.value_type = "float64"
        self.value = np.random.randint(1, 100, (5, 5, 5)).astype(self.value_type)
        self.index_type = "int64"
        self.index = np.zeros((5, 5, 5)).astype(self.index_type)
        self.axis = 1
        self.axis_type = "int64"
            ...
            for j in range(10):
                for k in range(10):
                    if self.nums[i, j, k] > 0:
                        self.target[i, j, k] = self.target[i, j, k] / self.nums[i, j, k]
        self.inputs = {
            "Input": self.xnp,
            "Index": self.index,
            "Value": self.value,
...
class TestPutAlongAxisAPICase4(unittest.TestCase):
    def setUp(self):
        ...
        self.value = np.arange(1, 11).reshape(self.value_shape).astype(np.float32)
        self.place = []
        if (
            os.environ.get("FLAGS_CI_both_cpu_and_gpu", "False").lower()
            in ["1", "true", "on"]
    ...
    def test_api_dygraph(self):
        ...
        def run_inplace(place):
            ...
            x_tensor = paddle.to_tensor(self.x_np)
            index_tensor2 = paddle.to_tensor(self.index_np2)
            x_tensor.put_along_axis_(index_tensor2, 10, 1, "assign", True, False)
            out_ref = copy.deepcopy(self.x_np)
            for i in range(self.index2_shape[0]):
                for j in range(self.index2_shape[1]):
    ...
    def test_error(self):
        ...
        try:
            res = paddle.put_along_axis(tensorx, indices, 1.0, 0, "assign", True, False)
    ...
    def test_index_type_error(self):
        ...
        with self.assertRaises(TypeError):
            res = paddle.put_along_axis(tensorx, indices, values, 0, "mul", True, False)
    ...
    def test_dynamic_static(self):
        with dygraph_guard():
            ...
            for ref, actual in zip(dy_out, st_out):
                np.testing.assert_allclose(ref, actual, rtol=self.tol, atol=self.tol)
