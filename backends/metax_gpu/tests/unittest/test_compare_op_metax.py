...
for _type_name in {
    ...
    create_test_class("greater_equal", _type_name, lambda _a, _b: _a >= _b, True)
    ...
    class PaddleCls(unittest.TestCase):
        ...
        def test_not_equal(self):
            if self.op_type == "not_equal":
                paddle.disable_static()
                x = paddle.to_tensor(np.array([1.2e-15, 2, 2, 1]), dtype="float32")
                y = paddle.to_tensor(np.array([1.1e-15, 2, 2, 1]), dtype="float32")
                op = eval(f"paddle.{self.op_type}")
                out = op(x, y)
