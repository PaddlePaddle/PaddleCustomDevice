...
class TestElementwiseAddOpAutoParallelXShardBroadcast(TestElementwiseAddOpAutoParallel):
    def init_placements(self):
        self.placements = {
            "X": [dist.Shard(0)],
            "Y": [dist.Replicate()],
...
class TestElementwiseAddOpAutoParallelXYShard(TestElementwiseAddOpAutoParallel):
    ...
    def test_check_grad(self):
        place = core.CUDAPlace(0)
        self.check_grad_with_place(place, ["X", "Y"], "Out", check_auto_parallel=True)
