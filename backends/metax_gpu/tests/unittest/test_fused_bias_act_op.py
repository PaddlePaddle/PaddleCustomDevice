...
def fake_quant(values, shift, smooth, quant_scale, max_bound, min_bound, round_type):
...
class TestFusedBiasActOp(unittest.TestCase):
    ...
    def generate_inputs(self):
        self.x = (np.random.rand(self.batch_size, self.seq_len, self.cols) * 16).astype(
            self.dtype
        )
    ...
    def compute_paddle_output(self):
        paddle.disable_static(place=paddle.CustomPlace("metax_gpu", 0))
...
class TestFusedBiasActOpBF16(unittest.TestCase):
    ...
    def generate_inputs(self):
        self.x = (
            np.random.rand(self.batch_size, self.seq_len, self.cols).astype("float32")
            * 16
        )
...
class TestAssert(unittest.TestCase):
    ...
    def test_assert_case1(self):
        paddle.disable_static(place=paddle.CustomPlace("metax_gpu", 0))
        x = np.random.randint(low=-16, high=16, size=(self.rows, self.cols)).astype(
            "int32"
        )
