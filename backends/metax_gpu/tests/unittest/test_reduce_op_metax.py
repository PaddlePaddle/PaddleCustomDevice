...
class TestMin6DOp(OpTest):
    ...
    def setUp(self):
        self.op_type = "reduce_min"
        self.python_api = paddle.min
        self.inputs = {"X": np.random.random((2, 4, 3, 5, 6, 10)).astype("float64")}
        self.attrs = {"dim": [2, 4]}
        self.outputs = {"Out": self.inputs["X"].min(axis=tuple(self.attrs["dim"]))}
...
@unittest.skipIf(not paddle.is_compiled_with_cuda(), "FP16 test runs only on GPU")
...
class TestProd8DFP16OP(TestProd8DOp):
    ...
    def test_check_grad(self):
        self.check_grad_with_place(paddle.CUDAPlace(0), ["X"], "Out", check_pir=True)
...
class API_TestSumOp(unittest.TestCase):
    ...
    def test_static(self):
        ...
        self.run_static(shape, "int32", (), attr_dtype="int32", np_axis=(0, 1, 2))
...
class TestAllAPI(unittest.TestCase):
    ...
    def check_static_float_result(self, place):
        ...
        startup = paddle.static.Program()
        with base.program_guard(main, startup):
            input = paddle.static.data(name="input", shape=[4, 4], dtype="float")
            result = paddle.all(x=input)
...
class TestAllZero(unittest.TestCase):
    ...
    def check_result(self, static_result, expected_result, axis, keepdim, dtype, place):
        self.assertTrue(
    ...
    def _test_static(self, place, axis, keepdim, dtype):
        with static_guard():
            with base.program_guard(paddle.static.Program(), paddle.static.Program()):
                ...
                expected_result = self.calculate_expected_result(x_np, axis, keepdim)
                self.check_result(
