...
// #include "kernels/funcs/mlu_op_runner.h"
...
std::vector<paddle::Tensor> mlu_rms_norm(const paddle::Tensor& x,
  ...
  MLUCnnlTensorDesc x_desc(x_tensor);
  MLUCnnlTensorDesc gamma_desc(gamma_tensor);
  MLUCnnlTensorDesc y_desc(y_tensor);
  MLUCnnlTensorDesc rstd_desc(rstd_tensor);

  MLUCnnl::RmsNormForward(dev_ctx,
                          rstd_dim,
                          x_desc.get(),
                          GetBasePtr(&x_tensor),
                          gamma_desc.get(),
                          GetBasePtr(&gamma_tensor),
                          nullptr,
                          eps,
                          y_desc.get(),
                          GetBasePtr(y_tensor),
                          rstd_desc.get(),
                          GetBasePtr(rstd_tensor));
