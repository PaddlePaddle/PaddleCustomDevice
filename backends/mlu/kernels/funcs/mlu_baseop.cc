...
const std::shared_ptr<MLUCnnlRandomGeneratorDesc>& GetMLURandomGenerator(
  ...
  static uint32_t num_mlu_devices = -1;
  ...
  if (device_id < 0) {
    PADDLE_THROW(
        phi::errors::InvalidArgument("mlu device id shoule be greater than 0"));
  }
...
MLUCnnlRandomGeneratorDesc::MLUCnnlRandomGeneratorDesc(const Context& ctx,
                                                       const int seed) {
  PADDLE_ENFORCE_MLU_SUCCESS(
...
void MLUCnnlBatchSpaceDesc::getSpace2batchNdextraInputSize(
    const Context& ctx, const cnnlTensorDescriptor_t input_desc) {
  cnnlHandle_t handle = GetHandleFromCTX(ctx);
...
NormalizeDesc::NormalizeDesc(int* axis,
                             int axis_num,
                             ...
                             int channel_shared,
                             int across_spatial) {
  PADDLE_ENFORCE_MLU_SUCCESS(cnnlCreateNormalizeDescriptor(&normalize_desc_));
  PADDLE_ENFORCE_MLU_SUCCESS(cnnlSetNormalizeDescriptor_v2(normalize_desc_,
                                                           axis,
                                                           axis_num,
                                                           nan_propagation,
                                                           eps,
...
const cnnlNormalizeDescriptor_t NormalizeDesc::get() const {
  return normalize_desc_;
}

NormalizeDesc::~NormalizeDesc() {
  if (normalize_desc_) {
    PADDLE_ENFORCE_MLU_SUCCESS(cnnlDestroyNormalizeDescriptor(normalize_desc_));
  }
}

/* static */ void MLUCnnl::Active(const Context& ctx,
...
/* static */ void MLUCnnl::ActiveGrad(const Context& ctx,
                                      cnnlActivationDescriptor_t active_desc,
                                      const void* alpha,
                                      const void* beta,
...
/* static */ void MLUCnnl::Concat(const Context& ctx,
                                  const int pack_num,
                                  const int axis,
...
/* static */ void MLUCnnl::Div(const Context& ctx,
...
/* static */ void MLUCnnl::Fill(const Context& ctx,
...
/* static */ void MLUCnnl::LRN(const Context& ctx,
...
/* static */ void MLUCnnl::Range(const Context& ctx,
                                 const void* start,
                                 const void* end,
                                 const void* step,
                                 const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Cumsum(const Context& ctx,
                                  const int axis,
...
/* static */ void MLUCnnl::BroadcastTo(const Context& ctx,
                                       const cnnlTensorDescriptor_t input_desc,
                                       const void* input,
                                       const cnnlTensorDescriptor_t output_desc,
                                       void* output) {
...
/* static */ void MLUCnnl::AssignAdd(const Context& ctx,
                                     const void* alpha,
                                     const void* beta,
...
/* static */ void MLUCnnl::AssignSub(const Context& ctx,
                                     const void* alpha,
                                     const void* beta,
                                     const cnnlTensorDescriptor_t update_desc,
                                     const void* update,
...
/* static */ void MLUCnnl::Assign(const Context& ctx,
                                  const cnnlTensorDescriptor_t update_desc,
                                  const void* update,
                                  const cnnlTensorDescriptor_t param_desc,
                                  void* param) {
...
/* static */ void MLUCnnl::SGD(const Context& ctx,
...
/* static */ void MLUCnnl::ApplyAdaGrad(const Context& ctx,
                                        const cnnlTensorDescriptor_t grad_desc,
                                        const void* grad,
...
/* static */ void MLUCnnl::ApplyRMSProp(const Context& ctx,
                                        const cnnlTensorDescriptor_t grad_desc,
                                        const void* grad,
                                        const void* lr,
                                        const void* rho,
...
/* static */ void MLUCnnl::ApplyAdam(const Context& ctx,
                                     const cnnlTensorDescriptor_t var_desc,
                                     void* var,
...
/* static */ void MLUCnnl::ApplyAdaMax(const Context& ctx,
                                       const cnnlTensorDescriptor_t grad_desc,
                                       const cnnlTensorDescriptor_t var_desc,
                                       void* var,
                                       const cnnlTensorDescriptor_t m_desc,
...
/* static */ void MLUCnnl::ApplyMomentum(const Context& ctx,
                                         const cnnlTensorDescriptor_t grad_desc,
                                         const void* grad,
                                         const bool use_nesterov,
                                         const void* lr,
...
/* static */ void MLUCnnl::ApplyAdadelta(const Context& ctx,
                                         const cnnlTensorDescriptor_t grad_desc,
                                         const void* diff,
                                         const void* lr,
                                         const void* rho,
...
/* static */ void MLUCnnl::Scale(const Context& ctx,
                                 const int axis,
                                 const cnnlTensorDescriptor_t input_desc,
                                 const void* input,
...
/* static */ void MLUCnnl::AddN(const Context& ctx,
                                uint32_t input_num,
                                const cnnlTensorDescriptor_t inputs_desc[],
                                const void* inputs[],
                                const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Log(const Context& ctx,
                               cnnlComputationPreference_t prefer,
...
/* static */ void MLUCnnl::Matmul(const Context& ctx,
...
/* static */ void MLUCnnl::BatchMatmul(const Context& ctx,
                                       const bool transpose_a,
                                       const bool transpose_b,
                                       const cnnlDataType_t data_type,
                                       const cnnlTensorDescriptor_t in0_desc,
...
/* static */ void MLUCnnl::MulAx(const Context& ctx,
                                 const cnnlTensorDescriptor_t alpha_desc,
                                 const void* alpha,
                                 const cnnlTensorDescriptor_t output_desc,
                                 void* output) {
...
/* static */ void MLUCnnl::Split(const Context& ctx,
                                 int split_num,
                                 int axis,
                                 const cnnlTensorDescriptor_t input_desc,
...
/* static */ void MLUCnnl::Logic(const Context& ctx,
...
/* static */ void MLUCnnl::Select(const Context& ctx,
...
/*static */ void MLUCnnl::GatherNd(const Context& ctx,
                                   const cnnlTensorDescriptor_t params_desc,
                                   const void* params,
                                   const cnnlTensorDescriptor_t indices_desc,
                                   const void* indices,
...
/* static */ void MLUCnnl::Softplus(const Context& ctx,
...
/* static */ void MLUCnnl::RsqrtGrad(const Context& ctx,
...
/* static */ void MLUCnnl::SqrtGrad(const Context& ctx,
                                    const cnnlTensorDescriptor_t data_desc,
                                    const void* y,
                                    const void* diff_y,
                                    void* output) {
...
/* static */ void MLUCnnl::Pad(const Context& ctx,
                               const cnnlTensorDescriptor_t input_desc,
                               const void* input,
                               const void* paddings,
...
/* static */ void MLUCnnl::OneHot(const Context& ctx,
...
/* static */ void MLUCnnl::Tile(const Context& ctx,
                                const cnnlTensorDescriptor_t input_desc,
                                const void* input,
                                const cnnlTensorDescriptor_t output_desc,
                                void* output) {
...
/* static */ void MLUCnnl::Reduce(const Context& ctx,
...
/* static */ void MLUCnnl::FloorDiv(const Context& ctx,
                                    cnnlComputationPreference_t prefer,
                                    const cnnlTensorDescriptor_t input1_desc,
                                    const void* input1,
                                    const cnnlTensorDescriptor_t input2_desc,
...
/* static */ void MLUCnnl::FloorMod(const Context& ctx,
                                    const cnnlTensorDescriptor_t input1_desc,
                                    const void* input1,
                                    const cnnlTensorDescriptor_t input2_desc,
                                    const void* input2,
...
/* static */ void MLUCnnl::Maximum(const Context& ctx,
                                   const cnnlTensorDescriptor_t input1_desc,
                                   const void* input1,
                                   const cnnlTensorDescriptor_t input2_desc,
                                   const void* input2,
...
/* static */ void MLUCnnl::Minimum(const Context& ctx,
                                   const cnnlTensorDescriptor_t input1_desc,
                                   const void* input1,
                                   const cnnlTensorDescriptor_t input2_desc,
                                   const void* input2,
...
/* static */ void MLUCnnl::Pow(const Context& ctx,
                               cnnlComputationPreference_t prefer,
                               const cnnlTensorDescriptor_t input1_desc,
                               const void* input1,
                               const cnnlTensorDescriptor_t input2_desc,
...
/* static */ void MLUCnnl::PowR(const Context& ctx,
                                cnnlComputationPreference_t prefer,
                                const cnnlTensorDescriptor_t input1_desc,
                                const void* input1,
                                const cnnlTensorDescriptor_t input2_desc,
...
/* static */ void MLUCnnl::DivNoNan(const Context& ctx,
                                    cnnlComputationPreference_t prefer,
                                    const cnnlTensorDescriptor_t input1_desc,
                                    const void* input1,
                                    const cnnlTensorDescriptor_t input2_desc,
...
/* static */ void MLUCnnl::L2Loss(const Context& ctx,
                                  const cnnlTensorDescriptor_t input_desc,
                                  const void* input,
                                  void* output) {
  cnnlHandle_t handle = GetHandleFromCTX(ctx);
...
/* static */ void MLUCnnl::Abs(const Context& ctx,
                               const cnnlTensorDescriptor_t input_desc,
                               const void* input,
                               const cnnlTensorDescriptor_t output_desc,
                               void* output) {
...
/* static */ void MLUCnnl::FlashAttentionBackward(
  ...
  void* workspace_ptr = ctx.Alloc(&workspace, DataType::INT8, workspace_size);
  PADDLE_ENFORCE_MLU_SUCCESS(
      cnnlFlashAttentionBackward_v2(handle,
                                    flash_atten_desc,
                                    diff_out_desc,
                                    diff_out,
                                    q_desc,
                                    ...
                                    csk_desc,
                                    cu_seqlens_k,
                                    /*alibi_slope_desc*/ nullptr,
                                    /*alibi_slope*/ nullptr,
                                    /*additive_attn_mask_desc*/ nullptr,
                                    /*additive_attn_mask*/ nullptr,
                                    rng_state,
                                    workspace_ptr,
                                    workspace_size,
                                    diff_query_desc,
...
/* static */ void MLUCnnl::Floor(const Context& ctx,
                                 const cnnlTensorDescriptor_t input_desc,
                                 const void* input,
                                 const cnnlTensorDescriptor_t output_desc,
                                 void* output) {
...
/* static */ void MLUCnnl::Ceil(const Context& ctx,
                                const cnnlTensorDescriptor_t input_desc,
                                const void* input,
                                const cnnlTensorDescriptor_t output_desc,
                                void* output) {
...
/* static */ void MLUCnnl::IsNan(const Context& ctx,
                                 const cnnlTensorDescriptor_t input_desc,
                                 const void* input,
                                 const cnnlTensorDescriptor_t output_desc,
                                 void* output) {
...
/* static */ void MLUCnnl::Square(const Context& ctx,
                                  const cnnlTensorDescriptor_t input_desc,
                                  const void* input,
                                  const cnnlTensorDescriptor_t output_desc,
                                  void* output) {
...
/* static */ void MLUCnnl::Sqrt(const Context& ctx,
                                cnnlComputationPreference_t prefer,
                                const cnnlTensorDescriptor_t input_desc,
                                const void* input,
                                const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Rsqrt(const Context& ctx,
                                 cnnlComputationPreference_t prefer,
                                 const cnnlTensorDescriptor_t input_desc,
                                 const void* input,
                                 const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Cos(const Context& ctx,
                               cnnlComputationPreference_t prefer,
                               const cnnlTensorDescriptor_t input_desc,
                               const void* input,
                               const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Sin(const Context& ctx,
                               cnnlComputationPreference_t prefer,
                               const cnnlTensorDescriptor_t input_desc,
                               const void* input,
                               const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Exp(const Context& ctx,
                               cnnlComputationPreference_t prefer,
                               const cnnlTensorDescriptor_t input_desc,
                               const void* input,
                               const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Sign(const Context& ctx,
                                const cnnlTensorDescriptor_t input_desc,
                                const void* input,
                                const cnnlTensorDescriptor_t output_desc,
                                void* output) {
...
/* static */ void MLUCnnl::IndexSelect(const Context& ctx,
                                       const int dim,
...
/* static */ void MLUCnnl::IsFinite(const Context& ctx,
                                    const cnnlTensorDescriptor_t input_desc,
                                    const void* input,
                                    const cnnlTensorDescriptor_t output_desc,
                                    void* output) {
...
/* static */ void MLUCnnl::IsNanInf(const Context& ctx,
                                    const cnnlTensorDescriptor_t input_desc,
                                    const void* input,
                                    void* output) {
  cnnlHandle_t handle = GetHandleFromCTX(ctx);
...
/* static */ void MLUCnnl::Erf(const Context& ctx,
                               cnnlComputationPreference_t prefer,
                               const cnnlTensorDescriptor_t input_desc,
                               const void* input,
                               const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Log1p(const Context& ctx,
                                 cnnlComputationPreference_t prefer,
                                 const cnnlTensorDescriptor_t input_desc,
                                 const void* input,
                                 const cnnlTensorDescriptor_t output_desc,
...
/* static */ void MLUCnnl::Interp(const Context& ctx,
...
/*static*/ void MLUCnnl::Clip(const Context& ctx,
                              const cnnlTensorDescriptor_t x_desc,
                              const void* x,
                              const void* min,
                              const void* max,
...
/* static */ void MLUCnnl::Normalize(
  ...
  cnnlHandle_t handle = GetHandleFromCTX(ctx);
  size_t workspace_size;
  PADDLE_ENFORCE_MLU_SUCCESS(cnnlGetNormalizeWorkspaceSize(handle,
                                                           normalize_desc,
                                                           input_desc,
                                                           output_desc,
                                                           norm_desc,
  ...
  PADDLE_ENFORCE_MLU_SUCCESS(cnnlNormalize_v3(handle,
                                              normalize_desc,
                                              input_desc,
                                              input,
                                              scale_desc,
...
/* static */ void MLUCnnl::Conv2D(const Context& ctx,
...
/* static */ void MLUCnnl::DCNForward(const Context& ctx,
...
/* static */ void MLUCnnl::Transpose(const Context& ctx,
...
/* static */ void MLUCnnl::TrilTriu(const Context& ctx,
...
/* static */ void MLUCnnl::NumTrue(const Context& ctx,
                                   const cnnlTensorDescriptor_t x_desc,
                                   const void* x,
...
/* static */ void MLUCnnl::Where(const Context& ctx,
                                 const cnnlTensorDescriptor_t x_desc,
                                 const void* x,
                                 const cnnlTensorDescriptor_t num_true_desc,
                                 const void* num_true,
...
/* static */ void MLUCnnl::InTopK(const Context& ctx,
...
/* static */ void MLUCnnl::ScatterNd(const Context& ctx,
...
/* static */ void MLUCnnl::BitWise(const Context& ctx,
...
/* static */ void MLUCnnl::QR(const Context& ctx,
                              const cnnlTensorDescriptor_t a_desc,
                              const void* a,
                              const cnnlTensorDescriptor_t q_desc,
                              void* q,
...
/* static */ void MLUCnnl::Reciprocal(const Context& ctx,
                                      const cnnlTensorDescriptor_t input_desc,
                                      const void* input,
                                      const cnnlTensorDescriptor_t output_desc,
                                      void* output) {
...
/* static */ void MLUCnnl::Transform(const Context& ctx,
                                     const void* alpha,
                                     const void* beta,
                                     const cnnlTensorDescriptor_t input_desc,
                                     const void* input,
...
/* static */ void MLUCnnl::RNNForward(const Context& ctx,
  ...
  PADDLE_ENFORCE_NOT_NULL(
      x_desc,
      phi::errors::Fatal("MLU RNNForward failed. x_desc initializing failed."));
...
/* static */ void MLUCnnl::RNNBackward(const Context& ctx,
                                       const cnnlRNNDescriptor_t rnn_desc,
...
/* static */ void MLUCnnl::Mask(const Context& ctx,
...
/* static */ void MLUCnnl::RoiAlign(const Context& ctx,
