...
cnnlPoolingMode_t ToCnnlPoolingMode(const std::string& pooling_type,
  ...
  if (pooling_type == "max") {
    ...
    PADDLE_THROW(
        phi::errors::InvalidArgument("Unknown pooling_type: %s", pooling_type));
  }
  return pooling_mode;
}
}  // namespace
...
void Pool2dKernel(const Context& dev_ctx,
  ...
  if (!adaptive) {
    ...
    if (extra_input_size > 0) {
      ...
      int8_t* h_extra_tensor_ptr =
          dev_ctx.template HostAlloc<int8_t>(&extra_host_tensor);
...
void Pool2dGradKernel(const Context& dev_ctx,
  ...
  if (pooling_type == "max") {
    ...
    // cnnlPoolingIndex supports any combinations of the following data types
    // for x tensor, y tensor. Note that the combinations of these tensors must
    // be half-int16 or float-int32.
    MLUCnnlTensorDesc index_tensor_desc;
    if (in_x.dtype() == DataType::FLOAT16) {
      dev_ctx.template Alloc<int16_t>(&index_tensor);
      index_tensor_desc = MLUCnnlTensorDesc(
          index_tensor, CNNL_LAYOUT_NHWC, ToCnnlDataType<int16_t>());
    } else {
