...
class TestDistPPTraning(unittest.TestCase):
    ...
    def test_pp_model(self):
        ...
        model_a, optimizer_a = paddle.amp.decorate(
            models=model_a, optimizers=optimizer_a, level="O2", save_dtype="float32"
        )
        model_b, optimizer_b = paddle.amp.decorate(
            models=model_b, optimizers=optimizer_b, level="O2", save_dtype="float32"
        )
        ...
        for step_id, data in enumerate(train_reader()):
            ...
            y_data = (
                np.array([x[1] for x in data]).astype("int64").reshape(batch_size, 1)
            )
            ...
            np.testing.assert_allclose(loss_a.numpy(), loss_b.numpy(), rtol=5e-3)
