...
class TestAdamWOpMultiPrecisonWithMainGrad(unittest.TestCase):
    def _test_adamw_op_dygraph_place_amp_with_maingrad(
        ...
        param = paddle.randn(shape).astype(paddle.float16)
        master_weight = param.astype(paddle.float32)
        grad = paddle.randn(shape).astype(paddle.float16)
        ...
        if use_main_grad:
            ...
            np.testing.assert_allclose(
                param.astype("float32").numpy(), ref_param.numpy(), atol=1e-5
            )
            np.testing.assert_allclose(
...
class TestAdamWOpMultiPrecisonWithMainGrad(unittest.TestCase):
    def _test_adamw_op_dygraph_place_amp_with_maingrad(
        ...
        if use_main_grad:
            ...
            np.testing.assert_allclose(
                param.astype("float32").numpy(), ref_param.numpy(), atol=1e-4
            )
            np.testing.assert_allclose(
