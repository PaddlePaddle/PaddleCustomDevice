    ...
    class Cls(OpTest):
        ...
        def test_errors(self):
            with paddle.pir_utils.OldIrGuard():
                ...
                if not paddle.get_flags(["FLAGS_enable_pir_api"])[
                    ...
                    def build_op(case):
                        with old_program_guard(OldProgram(), OldProgram()):
                            a = paddle.static.data(
                                name="a", shape=[-1, 2], dtype="float32"
                            )
                            b = paddle.static.data(
                                name="b", shape=[-1, 2], dtype="float32"
                            )
                            c = paddle.static.data(
                                name="c", shape=[-1, 2], dtype="int16"
                            )
                            d = base.create_lod_tensor(
                                np.array([[-1]]), [[1]], self.place
                            )
