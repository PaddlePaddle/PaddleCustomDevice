...
class TestFillConstantAPI(unittest.TestCase):
    def test_api(self):
        ...
        out_1 = fluid.layers.fill_constant(shape=[1, 2], dtype="float32", value=1.1)
        ...
        val1 = fluid.layers.fill_constant(shape=[1], dtype=np.float32, value=1.1)
        val2 = fluid.layers.fill_constant(shape=[1], dtype=np.float64, value=1.1)
...
class TestFillConstantImperative(unittest.TestCase):
    def test_api(self):
        with fluid.dygraph.guard():
            ...
            res1 = fluid.layers.fill_constant(shape=[1, 2], dtype="float32", value=1.1)
            res2 = fluid.layers.fill_constant(shape=shape, dtype="float32", value=1.1)
            res3 = fluid.layers.fill_constant(shape=shape, dtype="float32", value=val)
            res4 = fluid.layers.fill_constant(shape=shape, dtype="int32", value=value)
            assert np.array_equal(res1.numpy(), np.full([1, 2], 1.1, dtype="float32"))
            assert np.array_equal(res2.numpy(), np.full([1, 2], 1.1, dtype="float32"))
            assert np.array_equal(res3.numpy(), np.full([1, 2], 1.1, dtype="float32"))
            assert np.array_equal(res4.numpy(), np.full([1, 2], 88, dtype="int32"))
...
class TestFillConstantOpError(unittest.TestCase):
    def test_errors(self):
        with program_guard(Program(), Program()):
            ...
            self.assertRaises(
                TypeError, fluid.layers.fill_constant, shape=[1], value=5, dtype="uint4"
            )
            ...
            def test_shape_tensor_dtype():
                shape = fluid.data(name="shape_tensor", shape=[2], dtype="float32")
                fluid.layers.fill_constant(shape=shape, dtype="float32", value=1)

            self.assertRaises(TypeError, test_shape_tensor_dtype)

            def test_shape_tensor_list_dtype():
                shape = fluid.data(name="shape_tensor_list", shape=[1], dtype="bool")
                fluid.layers.fill_constant(shape=[shape, 2], dtype="float32", value=1)
