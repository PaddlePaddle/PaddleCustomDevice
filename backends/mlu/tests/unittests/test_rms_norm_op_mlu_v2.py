...
for lib in os.listdir(os.getenv("CUSTOM_DEVICE_ROOT")):
    print("os.listdir==: ", os.listdir(os.getenv("CUSTOM_DEVICE_ROOT")))
    if lib.endswith(".so"):
...
class TestMLURMSNormFP32(unittest.TestCase):
    def setUp(self):
    ...
    def fused_rms_norm(self, x, gamma, eps):
        y, rstd = core.eager._run_custom_op("rms_norm_mlu", x, gamma, eps)
        y.backward()
        dx = x.grad
        dgamma = gamma.grad
        if self.dtype == "bfloat16":
...
class TestMLURMSNormFP16(TestMLURMSNormFP32):
    def init_dtype(self):
        self.dtype = "float16"
...
class TestMLURMSNormBF16(TestMLURMSNormFP32):
    def init_dtype(self):
        self.dtype = "bfloat16"
