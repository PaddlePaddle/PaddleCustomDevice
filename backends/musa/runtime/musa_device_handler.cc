...
C_Status GetMusaDevicesList(size_t *devices) {
  ...
  //TODO(jihong.zhong): it should be realloc devices ptr, check it alloc mem by malloc or calloc before
...
C_Status MusaMemCpyP2PAsync(const C_Device dst_device,
                            ...
                            const void *src,
                            size_t size) {
  MUSA_CHECK(musaMemcpyPeerAsync(dst, dst_device->id, src, 
      src_device->id, size, musaStream_t(stream)));
  return C_SUCCESS;
}
...
C_Status GetMusaComputeCapability(const C_Device device,
  ...
  MUSA_CHECK(musaDeviceGetAttribute(&major_ver, musaDevAttrComputeCapabilityMajor, device->id));
  MUSA_CHECK(musaDeviceGetAttribute(&minor_ver, musaDevAttrComputeCapabilityMinor, device->id));
...
C_Status GetMusaMultiProcessorCount(const C_Device device, size_t *result) {
  int mp_cnt{0};
  MUSA_CHECK(musaDeviceGetAttribute(&mp_cnt, musaDevAttrMultiProcessorCount, device->id));
  *result = static_cast<size_t>(mp_cnt);
  return C_SUCCESS;
}

C_Status GetMusaMaxThreadsPerMP(const C_Device device,
                                size_t *result) {
...
C_Status GetMusaMaxThreadsPerBlock(const C_Device device,
                                   size_t *result) {
  int max_threads_per_block{0};
  MUSA_CHECK(musaDeviceGetAttribute(&max_threads_per_block, 
      musaDevAttrMaxThreadsPerBlock, device->id));
...
C_Status MusaInitEigenDevice(const C_Place place,
  ...
  EigenGpuStreamDevice *eigen_stream_ =
      new EigenGpuStreamDevice();
