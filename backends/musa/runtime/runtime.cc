...
void InitPlugin(CustomRuntimeParams *params) {
  ...
  params->interface->host_memory_deallocate = musa::device::MusaMemDeallocateHost;
  
  params->interface->unified_memory_allocate = nullptr; // TODO: fill it
  params->interface->unified_memory_deallocate = nullptr; // TODO: fill it

  // params->interface->device_memory_stats = DeviceMemStats;
  // params->interface->device_min_chunk_size = DeviceMinChunkSize;

  params->interface->get_compute_capability = musa::device::GetMusaComputeCapability;
  params->interface->get_device_properties = musa::device::GetMusaDeviceProperties;
  ...
  params->interface->get_multi_process = musa::device::GetMusaMultiProcessorCount;
  params->interface->get_max_threads_per_mp = musa::device::GetMusaMaxThreadsPerMP;
  params->interface->get_max_threads_per_block = musa::device::GetMusaMaxThreadsPerBlock;
  params->interface->get_max_grid_dim_size = musa::device::GetMusaMaxGridDimSize;

  params->interface->init_eigen_device = musa::device::MusaInitEigenDevice;
  params->interface->destroy_eigen_device = musa::device::MusaDestroyEigenDevice;
  ...
  // params->interface->is_bfloat16_supported = is_bf16_func; //TODO(jihong.zhong): fix it 
  ...
  params->interface->xccl_get_comm_name = [](C_CCLComm comm, char* comm_name)->C_Status {
