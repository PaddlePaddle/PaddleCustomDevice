...
std::vector<paddle::Tensor> npu_mm_allreduce(
  ...
  auto output_size = {x1.dims()[0], x2.dims()[1]};
  ...
  EXEC_NPU_CMD(aclnnMatmulAllReduce,
               *dev_ctx,
               x1_tensor,
               x2_tensor,
               *bias_real,
...
std::vector<std::vector<int64_t>> FusedMMAllReduceInferShape(
    std::vector<int64_t> x1_shape, std::vector<int64_t> x2_shape) {
  return {x1_shape, x2_shape};
}

PD_BUILD_OP(fused_mm_allreduce)
    .Inputs({"x1", "x2", paddle::Optional("bias")})
    .Outputs({"result"})
    .Attrs({"hcom:std::string", "reduce_op:std::string", "comm_turn:int64_t"})
    .SetKernelFn(PD_KERNEL(npu_mm_allreduce))
    .SetInferShapeFn(
        PD_INFER_SHAPE(FusedMMAllReduceInferShape));  // neccessary if the
                                                      // op has muti_inputs
