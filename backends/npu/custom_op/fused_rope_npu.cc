...
std::vector<paddle::Tensor> npufusedrope(
    ...
    if (numel <= 0) {
        PADDLE_ENFORCE_GE(numel,0,phi::errors::InvalidArgument("The number of the input 'query' for rope op must be a >=0 ""integer, but the value received is %d.",numel));
    }
    // query_out
    std::shared_ptr<phi::DenseTensor> query_out =std::make_shared<phi::DenseTensor>();
    query_out->Resize(query_tensor.dims());
    (*dev_ctx).Alloc(query_out.get(), query_tensor.dtype());
    auto head_dim = query_tensor.dims()[3];
    PADDLE_ENFORCE_EQ(head_dim % 64,0,phi::errors::InvalidArgument("The head_dim of input must be a multiple of 64."));
    ...
    PADDLE_ENFORCE_EQ(sin_dims,cos_dims,phi::errors::InvalidArgument("The dims of sin and cos must be the same. But ""recieved sin's dims is {%s}, cos's dims is {%s}.",sin_dims,cos_dims));
    PADDLE_ENFORCE_EQ(dims_size == 4,true,phi::errors::InvalidArgument("The dims of sin and cos is expected to ""be  4, but recieved %d.",dims_size));
    PADDLE_ENFORCE_EQ((sin_dims[0] == 1 && sin_dims[2] == 1),true,phi::errors::InvalidArgument("The batch_size and num_heads of sin and cos must be 1."));
...
std::vector<paddle::Tensor> npufusedropegrad(
    const paddle::Tensor& query,
    ...
    // query_grad
    std::shared_ptr<phi::DenseTensor> query_grad =std::make_shared<phi::DenseTensor>();
    query_grad->Resize(query_tensor.dims());
    (*dev_ctx).Alloc(query_grad.get(), query_tensor.dtype());

    // sin_grad
    std::shared_ptr<phi::DenseTensor> sin_grad =std::make_shared<phi::DenseTensor>();
    sin_grad->Resize(sin_tensor.dims());
    (*dev_ctx).Alloc(sin_grad.get(), sin_tensor.dtype());

    // cos_grad
    std::shared_ptr<phi::DenseTensor> cos_grad =std::make_shared<phi::DenseTensor>();
...
PD_BUILD_OP(fused_rope)
    ...
    .SetInferShapeFn(PD_INFER_SHAPE(
        fusedropeInferShape));  // neccessary if the op has muti_inputs

PD_BUILD_GRAD_OP(fused_rope)
    .Inputs({"query",
             paddle::Grad("query_out"),
             "cos",
             "sin"})
