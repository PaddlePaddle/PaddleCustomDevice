...
FusedFaPaGlobalVar &FusedFaPaGlobalVar::Instance() {
  static FusedFaPaGlobalVar ins;
  return ins;
}

void FusedFaPaGlobalVar::update_seqlens_encoder(
...
void FusedFaPaGlobalVar::update_seqlens_decoder(
    const phi::CustomContext &dev_ctx, const paddle::Tensor &seqlen) {
...
void FusedFaPaGlobalVar::update_block_tables(
...
void FusedFaPaGlobalVar::update_rope_encoder(const phi::CustomContext &dev_ctx,
  ...
  dev_ctx.template Alloc<phi::bfloat16>(g_rope_emb_encoder.rope_emb_cos.get());

  g_rope_emb_encoder.rope_emb_sin = std::make_shared<phi::DenseTensor>();
  g_rope_emb_encoder.rope_emb_sin->Resize(
      {g_seqlens_encoder.ntokens, head_dim});
  dev_ctx.template Alloc<phi::bfloat16>(g_rope_emb_encoder.rope_emb_sin.get());
  ...
  void *sin_data =
      cos_data + rope_emb.numel() / 2 * phi::SizeOf(phi::DataType::BFLOAT16);
  ...
  for (auto i = 0; i < seqlens_size; ++i) {
    if (seqlens[i] > 0) {
      ...
      AsyncMemCpyD2D(
          &device,
          stream,
          new_cos_data + out_offset * phi::SizeOf(phi::DataType::BFLOAT16),
          cos_data + in_offset * phi::SizeOf(phi::DataType::BFLOAT16),
          numel * phi::SizeOf(phi::DataType::BFLOAT16));
      AsyncMemCpyD2D(
          &device,
          stream,
          new_sin_data + out_offset * phi::SizeOf(phi::DataType::BFLOAT16),
          sin_data + in_offset * phi::SizeOf(phi::DataType::BFLOAT16),
          numel * phi::SizeOf(phi::DataType::BFLOAT16));
    }
  }
}

void FusedFaPaGlobalVar::update_rope_decoder(const phi::CustomContext &dev_ctx,
                                             const paddle::Tensor &rope_emb,
                                             int64_t max_seqlen,
                                             int64_t head_dim) {
  ...
  dev_ctx.template Alloc<phi::bfloat16>(g_rope_emb_decoder.rope_emb_cos.get());

  g_rope_emb_decoder.rope_emb_sin = std::make_shared<phi::DenseTensor>();
  g_rope_emb_decoder.rope_emb_sin->Resize(
      {g_seqlens_decoder.ntokens, head_dim});
  dev_ctx.template Alloc<phi::bfloat16>(g_rope_emb_decoder.rope_emb_sin.get());
...
void FusedFaPaGlobalVar::update_slots_encoder(const phi::CustomContext &dev_ctx,
                                              int64_t block_size,
...
void FusedFaPaGlobalVar::update_slots_decoder(const phi::CustomContext &dev_ctx,
                                              int64_t block_size,
                                              int64_t max_block_num) {
  if (g_seqlens_decoder.ntokens == 0) {
    return;
...
void FusedFaPaGlobalVar::update_casual_mask(const phi::CustomContext &dev_ctx,
  ...
  if (g_mask->numel() != max_seq_len * max_seq_len) {
    ...
    dev_ctx.template Alloc<phi::bfloat16>(g_mask.get());

    phi::DenseTensor ones_tensor;
    custom_kernel::FullKernel<phi::bfloat16>(dev_ctx,
    ...
    custom_kernel::TrilKernel<phi::bfloat16>(
        dev_ctx, ones_tensor, 0, &tril_ones_tensor);

    phi::DenseTensor tmp_mask;
    tmp_mask.Resize({max_seq_len, max_seq_len});
    custom_kernel::ScaleKernel<phi::bfloat16>(
        dev_ctx, tril_ones_tensor, -1.0f, 1.0f, true, g_mask.get());
  }
}

void FusedFaPaGlobalVar::update_in_encoder(const phi::CustomContext &dev_ctx,
  ...
  dev_ctx.template Alloc<phi::bfloat16>(g_out_encoder.first.get());

  g_out_encoder.second = std::make_shared<phi::DenseTensor>();
  g_out_encoder.second->Resize({ntokens, emb_dim});
  dev_ctx.template Alloc<phi::bfloat16>(g_out_encoder.second.get());
  ...
  for (auto i = 0; i < batch_size; ++i) {
    if (seqlens_encoder[i] > 0) {
      ...
      ACL_CHECK(
          aclrtMemcpyAsync(out_data + out_offset * sizeof(phi::bfloat16),
                           numel * sizeof(phi::bfloat16),
                           in_data + in_offset * sizeof(phi::bfloat16),
                           numel * sizeof(phi::bfloat16),
...
void FusedFaPaGlobalVar::update_in_decoder(const phi::CustomContext &dev_ctx,
                                           const paddle::Tensor &hidden) {
  if (g_seqlens_decoder.ntokens == 0) {
    return;
  }
  ...
  dev_ctx.template Alloc<phi::bfloat16>(g_out_decoder.first.get());

  g_out_decoder.second = std::make_shared<phi::DenseTensor>();
  g_out_decoder.second->Resize({ntokens, emb_dim});
  dev_ctx.template Alloc<phi::bfloat16>(g_out_decoder.second.get());
...
void FusedFaPaGlobalVar::update_out_encoder(const phi::CustomContext &dev_ctx,
  ...
  for (auto i = 0; i < ntokens; ++i) {
    ...
    ACL_CHECK(
        aclrtMemcpyAsync(out_data + out_offset * sizeof(phi::bfloat16),
                         emb_dim * sizeof(phi::bfloat16),
                         in_data + out_offset * sizeof(phi::bfloat16),
                         emb_dim * sizeof(phi::bfloat16),
                         ACL_MEMCPY_DEVICE_TO_DEVICE,
                         reinterpret_cast<aclrtStream>(dev_ctx.stream())));
  }
...
void FusedFaPaGlobalVar::update_out_decoder(const phi::CustomContext &dev_ctx,
                                            bool first_or_second,
                                            paddle::Tensor *out) {
  if (g_seqlens_decoder.ntokens == 0) {
    return;
...
void FusedFaPaGlobalVar::update_qkv_deq_offset(
...
void FusedFaPaGlobalVar::update_out_deq_offset(
    const phi::CustomContext &dev_ctx, int64_t sz) {
...
void FusedFaPaGlobalVar::update_ffn1_deq_offset(
    const phi::CustomContext &dev_ctx, int64_t sz) {
...
void FusedFaPaGlobalVar::update_ffn2_deq_offset(
    const phi::CustomContext &dev_ctx, int64_t sz) {
