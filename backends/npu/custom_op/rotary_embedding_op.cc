...
void fused_get_rotary_embedding_neox(
...
void fused_get_rotary_embedding(
    const int64_t* position_ids, 
    const int32_t bsz, 
    const int32_t max_seq_length, 
    const int32_t max_position_seq_length,
...
std::vector<paddle::Tensor> GetRoPE(
  ...
  auto rotary_embedding = paddle::full(out_shape, -1, paddle::DataType::FLOAT32, paddle::CPUPlace());
