...
void AbsKernel(const Context& dev_ctx,
               ...
               phi::DenseTensor* out) {
    dev_ctx.template Alloc<T>(out);
    DO_COMPATIBILITY(aclnnAbs, (custom_kernel::AbsKernelOp<T, Context>(dev_ctx, x, out)));
    auto size = sizeof(T);
...
void AbsGradKernel(const Context& dev_ctx,
    ...
    dev_ctx.template Alloc<T>(dx);
    auto size = sizeof(T);
    DO_COMPATIBILITY(aclnnSign, (custom_kernel::AbsGradKernelOp<T, Context>(dev_ctx, x, dout, dx)));
