...
FormatShape InferShapeNCDHWToND(FormatShape storage_dims, FormatShape base_dims);
FormatShape InferShapeNDToNCHW(FormatShape storage_dims, FormatShape base_dims);
FormatShape InferShapeNDToNCDHW(FormatShape storage_dims, FormatShape base_dims);
...
FormatShape FormatHelper::GetStorageShape(const aclFormat storage_format, const FormatShape origin_dims) {
  ...
  // LOG(FATAL) << "unsupport InferShape with format " << GetFormatName(storage_format) << "with shape" << GetShapeString(origin_dims);
  return {};
}
...
FormatShape InferShapeLessTo4(FormatShape dims) {
  FormatShape res;
  res.resize(4);
  PADDLE_ENFORCE_LE(dims.size(), 4, phi::errors::InvalidArgument("input dim > 4 when InferShapeLessTo4"));
  switch (dims.size()) {
    case 0:
      res[0] = 1;
      res[1] = 1;
    ...
    case 4:
      ...
      break;
    default:
      LOG(FATAL) << "dims of NCHW shape should not be greater than 4, which is " << dims.size();
  }
  return res;
}
...
FormatShape InferShapeNCHWToND(FormatShape storage_dims, FormatShape base_dims) {
  ...
  PADDLE_ENFORCE_EQ(cur_storage_dims.size(), 4,
                    phi::errors::InvalidArgument("input dim num not equal 4 when InferShapeNCHWToND"));
  ...
  switch (base_dims.size()) {
    case 1:
      ...
      PADDLE_ENFORCE_EQ(cur_storage_dims[0], 1, phi::errors::InvalidArgument("reshape type RESHAPE_TYPE_C erase dim N must be 1"));
      PADDLE_ENFORCE_EQ(cur_storage_dims[2], 1, phi::errors::InvalidArgument("reshape type RESHAPE_TYPE_C erase dim H must be 1"));
      PADDLE_ENFORCE_EQ(cur_storage_dims[3], 1, phi::errors::InvalidArgument("reshape type RESHAPE_TYPE_C erase dim W must be 1"));
      break;
    case 2:
      ...
      PADDLE_ENFORCE_EQ(cur_storage_dims[0], 1, phi::errors::InvalidArgument("reshape type RESHAPE_TYPE_CH erase dim N must be 1"));
      PADDLE_ENFORCE_EQ(cur_storage_dims[3], 1, phi::errors::InvalidArgument("reshape type RESHAPE_TYPE_CH erase dim W must be 1"));
      break;
    case 3:
      ...
      PADDLE_ENFORCE_EQ(cur_storage_dims[0], 1, phi::errors::InvalidArgument("reshape type RESHAPE_TYPE_CHW erase dim N must be 1"));
      break;
    case 4:
...
FormatShape InferShapeNDToNCHW(FormatShape storage_dims, FormatShape base_dims) {
  PADDLE_ENFORCE_LE(storage_dims.size(), 4, phi::errors::InvalidArgument("input storage dim not less than 4"));
  PADDLE_ENFORCE_LE(base_dims.size(), 4, phi::errors::InvalidArgument("input storage dim not less than 4"));
  return InferShapeLessTo4(base_dims);
}

FormatShape InferShapeNDToNCDHW(FormatShape storage_dims, FormatShape base_dims) {
  PADDLE_ENFORCE_EQ(storage_dims.size(), 5, phi::errors::InvalidArgument("ND failed to convert to NCDHW"));
  FormatShape res;
  res.resize(5);
  res = storage_dims;
  return res;
...
FormatShape InferShapeNCDHWToND(FormatShape storage_dims, FormatShape base_dims) {
  ...
  res.resize(5);
  res = storage_dims;
  PADDLE_ENFORCE_EQ(res.size(), 5, phi::errors::InvalidArgument("input dim num not equal 5 when InferShapeNCDHWToND"));
  return res;
}
