...
void NpuOpRunner::InitFloatStatus(aclrtStream stream) const {
  // Init float_status_desc_ and float_status_buffer once, if
  // check_nan_inf is needed, only do ClearFloatStatus here.
...
void NpuOpRunner::Run(aclrtStream stream, bool sync) const {
  ...
  if (FLAGS_ascend_check_nan_inf && GetFloatStatus(stream)) {
    PrintOpInfo();
    PADDLE_THROW(phi::errors::PreconditionNotMet("Operator %s contains Nan/Inf.", op_type_));
  }
}
