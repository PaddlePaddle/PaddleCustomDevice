...
void MaskedSelectKernel(const Context& dev_ctx,
  ...
  dev_ctx.template Alloc<int32_t>(&out_size);
  {
    const auto& cast_runner = NpuOpRunner(
        ...
        {{"dst_type",
          static_cast<int32_t>(
              ConvertToNpuDtype(paddle::experimental::DataType::INT32))}});
    ...
    sum_runner.AddInput(dev_ctx, std::vector<int32_t>({0}));
  ...
  phi::DenseTensor topkv2_out, indices;
...
void MaskedSelectGradKernel(const Context& dev_ctx,
  ...
  dev_ctx.template Alloc<int32_t>(&indices);
  {
    ...
    scatter_runner.AddInput(
        dev_ctx, std::vector<int32_t>({static_cast<int32_t>(x_grad->numel())}));
