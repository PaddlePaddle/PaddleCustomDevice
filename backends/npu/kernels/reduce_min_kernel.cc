...
template <typename T, typename Context>
void AclopMinGradKernel(const Context& dev_ctx,
                        const phi::DenseTensor& x,
...
template <typename T, typename Context>
void MinGradKernel(const Context& dev_ctx,
                   const phi::DenseTensor& x,
                   const phi::DenseTensor& out,
                   const phi::DenseTensor& out_grad,
                   const phi::IntArray& reduce_dims_in,
  ...
  DO_COMPATIBILITY(
      aclnnExpand,
      (custom_kernel::AclopMinGradKernel<T, Context>(dev_ctx,
                                                     x,
                                                     out,
                                                     out_grad,
                                                     reduce_dims_in,
...
PD_REGISTER_PLUGIN_KERNEL(min_grad,
                          npu,
                          ALL_LAYOUT,
                          custom_kernel::MinGradKernel,
                          int32_t,
                          int64_t,
                          phi::dtype::float16,
                          phi::dtype::bfloat16,
