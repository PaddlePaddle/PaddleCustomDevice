...
void SequenceMaskKernel(
  ...
  if (maxlen == -1) {
    ...
    if (x_numel == 0) {
      ...
      maxlen =
          static_cast<int>(*std::max_element(local_data, local_data + x_numel));
    }
  }
  PADDLE_ENFORCE_GT(
      maxlen,
  ...
  if (nx.dtype() == phi::DataType::FLOAT64 ||
    ...
    const auto& cast_runner1 = NpuOpRunner(
        "Cast", {nx}, {tn_x}, {{"dst_type", static_cast<int>(float_dtype)}});
  ...
  mk_runner.SetType("Range")
      .AddInput(dev_ctx, std::move(std::vector<T>{static_cast<int>(0)}), false)
      .AddInput(
          dev_ctx, std::move(std::vector<T>{static_cast<int>(maxlen)}), false)
      .AddInput(dev_ctx, std::move(std::vector<T>{static_cast<int>(1)}), false)
      .AddOutput(range_vec);
  ...
  if (range_vec.dtype() == phi::DataType::FLOAT64 ||
    ...
    const auto& cast_runner2 =
        NpuOpRunner("Cast",
                    {range_vec},
                    {t_range_vec},
                    {{"dst_type", static_cast<int>(float_dtype)}});
  ...
  const auto& ep_runner2 =
      NpuOpRunner("ExpandD", {t_range_vec}, {range_mask}, attr_input2);
  ...
  const auto& runner =
      NpuOpRunner("Greater", {mask_x, range_mask}, {bool_out}, {});
  runner.Run(stream);
