...
void TileKernelImpl(const Context& dev_ctx,
                    ...
                    int rank,
                    phi::DenseTensor* out) {
  DO_COMPATIBILITY(aclnnRepeat,
                   (custom_kernel::AclopTileKernelImpl<T, Context>(
                       dev_ctx, x, repeat_times, rank, out)));
  ...
  EXEC_NPU_CMD(aclnnRepeat, dev_ctx, x, repeat_times, *out);
}
...
template <typename T, typename Context>
void AclopTileGradKernelImpl(const Context& dev_ctx,
...
void TileGradKernelImpl(const Context& dev_ctx,
  ...
  if (out_grad.dtype() != phi::DataType::BOOL) {
    phi::DenseTensor dout(out_grad);
    if (reshape_dims_vec.size() >= 10) {
      // paddle only supports access to dimension 0 to 9 by 2025/1/16
      // so we need to split the dims into two parts, and then do sum twice.
      bool keep_dim = false;
      ...
      phi::IntArray reduce_dims_arry_second(reduce_dims_vec_second);
