 ...
 */
#include "token_penalty_multi_scores_v2_tiling.h"
...
namespace optiling {
    static ge::graphStatus TokenPenaltyMultiScoresV2TilingFunc(gert::TilingContext *context)
    {
        TokenPenaltyMultiScoresV2TilingData tiling;
        const gert::StorageShape* preIdsShape = context->GetInputShape(0);
        const gert::StorageShape* logitsShape = context->GetInputShape(1);
        const gert::StorageShape* badWordsShape = context->GetInputShape(7);
        const gert::StorageShape* endTokenIdShape = context->GetInputShape(10);
        ...
        int badWLen = badWordsShape->GetStorageShape().GetDim(0);
        ...
        if (vs % blockElements != 0) {
            std::cout << "[ERROR] TokenPenaltyMultiScoresV2 voc_size " << vs << " is invalid" << std::endl;
        ...
        std::cout << "[INFO] TokenPenaltyMultiScoresV2 tiling result, vs " << vs
            << ", vsBlock " << vsBlock << ", seqLen " << seqLen << ", etil " << etiLength
            << ", bs " << bs << ", bsBlock " << coreLoop << ", badTokenLen " << badWLen <<std::endl;
...
namespace ge {
    static ge::graphStatus TokenPenaltyMultiScoresV2InferShape(gert::InferShapeContext *context)
    {
    ...
    static ge::graphStatus TokenPenaltyMultiScoresV2InferShapeRange(gert::InferShapeRangeContext *context)
    {
    ...
    static ge::graphStatus TokenPenaltyMultiScoresV2InferDataType(gert::InferDataTypeContext *context)
    {
...
namespace ops {
    class TokenPenaltyMultiScoresV2 : public OpDef {
    public:
        explicit TokenPenaltyMultiScoresV2(const char *name) : OpDef(name)
        {
            this->Input("preIds")
                .ParamType(REQUIRED)
            ...
            this->SetInferShape(ge::TokenPenaltyMultiScoresV2InferShape)
                .SetInferShapeRange(ge::TokenPenaltyMultiScoresV2InferShapeRange)
                .SetInferDataType(ge::TokenPenaltyMultiScoresV2InferDataType);

            this->AICore()
                .SetTiling(optiling::TokenPenaltyMultiScoresV2TilingFunc);
    ...
    OP_ADD(TokenPenaltyMultiScoresV2);
}