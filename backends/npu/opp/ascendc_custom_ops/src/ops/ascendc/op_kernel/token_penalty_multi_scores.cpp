...
class TokenPenaltyMultiScores {
public:
    __aicore__ inline TokenPenaltyMultiScores(
        int32_t vs, int32_t vsBlock, int32_t sl, int32_t etil, int32_t bs, int32_t bsBlock)
    {
        vocabSize_ = vs;
...
extern "C" __global__ __aicore__ void token_penalty_multi_scores(
    GM_ADDR preIds, GM_ADDR logits, GM_ADDR repeatTimes, GM_ADDR penaltyScores, GM_ADDR frequencyScores,
    GM_ADDR presenceScores, GM_ADDR curLen, GM_ADDR minLen, GM_ADDR eosTokenId, 
    GM_ADDR logitsOut, GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tilingData, tiling);
    TokenPenaltyMultiScores op(
        tilingData.vs, tilingData.vsBlock, tilingData.seqLen, tilingData.etil, tilingData.bs, tilingData.bsBlock);
    op.Init(preIds, logits, repeatTimes, penaltyScores, frequencyScores, presenceScores, curLen, minLen, eosTokenId, logitsOut);
    op.Process();
}
