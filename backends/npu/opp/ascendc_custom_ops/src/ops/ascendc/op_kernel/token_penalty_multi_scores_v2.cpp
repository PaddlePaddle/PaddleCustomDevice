...
class TokenPenaltyMultiScoresV2 {
public:
    __aicore__ inline TokenPenaltyMultiScoresV2(
        int32_t vs, int32_t vsBlock, int32_t sl, int32_t etil, int32_t bs, int32_t bsBlock, int32_t badWordsLen)
    {
        vocabSize_ = vs;
        vocabLocalSize_ = vsBlock;
        seqLen_ = sl;
        eosTokenIdLength_ = etil;
        bs_ = bs;
        bsEachCore_ = bsBlock;
        badWLen_ = badWordsLen;
    }
    ...
    __aicore__ inline void Init(__gm__ uint8_t *preIds,
                                ...
                                __gm__ uint8_t *temperatures,
                                __gm__ uint8_t *badWords,
                                ...
                                __gm__ uint8_t *logitsOut)
    {
        ...
        temperaturesGm_ = (__gm__ float *)temperatures;
        badTokenIdGm_ = (__gm__ int64_t *)badWords;
        ...
        pipe_.InitBuffer(temperaturesTmpBuff_, vocabLocalSize_ * sizeof(float));
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int32_t bsIdEachCore = 0; bsIdEachCore < bsEachCore_; bsIdEachCore++) {
            ...
            for (int32_t i = 0; i < vocLoop; i++) {
                ...
                int32_t gmOffset = bsOffset * vocabSize_;
                for (int32_t j = 0; j < badWLen_; j++) {
                    int64_t badTokenId = *(badTokenIdGm_ + j);
                    pipe_barrier(PIPE_ALL);
                    if (badTokenId >= static_cast<int64_t>(vocabSize_) || badTokenId < 0) {
                        continue;
                    }
                    *(logitsOutGm_ + gmOffset + badTokenId) = -1e10f;
                    pipe_barrier(PIPE_ALL);
                }
            }
        }
    ...
    __aicore__ inline void Compute(int32_t bsId, int32_t vocSizeId, int32_t vocabLocalSize_)
    {
        ...
        if (*(curLenGm_ + bsId) >= 0) {
            ...
            if (*(curLenGm_ + bsId) < *(minLenGm_ + bsId)) {
                for (int i = 0; i < eosTokenIdLength_; ++i) {
        ...
        LocalTensor<float> temperaturesTmpL = temperaturesTmpBuff_.Get<float>();
        ...
        float temperatures = *(temperaturesGm_ + bsId);
        float temperaturesR = 1.0f / temperatures;
        ...
        Muls(logitsOutL, logitsOutL, temperaturesR, vocabLocalSize_);
...
private:
    ...
    __gm__ float *temperaturesGm_;
    __gm__ int64_t *badTokenIdGm_;
    ...
    TBuf<QuePosition::VECCALC> temperaturesTmpBuff_;
...
/**
    0   preIds: bs, length_id
    1   logits: bs, length
    2   repeatTimes: bs, length
    3   penaltyScores: bs
    4   frequencyScores: bs
    5   presenceScores: bs
    6   temperatures: bs
    7   badWords: bs, badWordsLen
    8   curLen: bs
    9   minLen: bs
    10  eosTokenId: end_length, 

    0   logitsOut: bs, length
*/
extern "C" __global__ __aicore__ void token_penalty_multi_scores_v2(
    GM_ADDR preIds, GM_ADDR logits, GM_ADDR repeatTimes, GM_ADDR penaltyScores, GM_ADDR frequencyScores,
    GM_ADDR presenceScores, GM_ADDR temperatures, GM_ADDR badWords, GM_ADDR curLen, GM_ADDR minLen, GM_ADDR eosTokenId,
    GM_ADDR logitsOut, GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tilingData, tiling);
    TokenPenaltyMultiScoresV2 op(
        tilingData.vs, tilingData.vsBlock, tilingData.seqLen, tilingData.etil, tilingData.bs, tilingData.bsBlock, tilingData.badWLen);
    op.Init(preIds, logits, repeatTimes, penaltyScores, frequencyScores, presenceScores, temperatures, badWords, curLen, minLen, eosTokenId, logitsOut);
    op.Process();
}