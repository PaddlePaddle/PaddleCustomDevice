...
class MLP(paddle.nn.Layer):
    def __init__(self, linear_size=100, param_attr=None, bias_attr=None):
        super().__init__()
...
class RandomDataset(paddle.io.Dataset):
    def __init__(self, num_samples=100, linear_size=100):
...
def train_mlp(
    ...
    if sharding_stage == 2:
        group = paddle.distributed.new_group([0, 1], backend="xccl")

        optimizer = GroupShardedOptimizerStage2(
            params=optimizer._parameter_list, optim=optimizer, group=group, device="npu"
        )

        model = GroupShardedStage2(
            model, optimizer, group=group, buffer_max_size=2**21, device="npu"
        )
    else:
