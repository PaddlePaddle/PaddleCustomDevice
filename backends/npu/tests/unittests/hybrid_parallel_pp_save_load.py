...
class TestDistPPSaveLoadTraning(unittest.TestCase):
    ...
    def test_pp_model(self):
        ...
        paddle.save(
            optimizer.state_dict(), os.path.join(output_dir, "model_state.pdopt")
        )
        ...
        for step_id in range(5):
            ...
            x.stop_gradient = True
            loss = model.train_batch([x, x], optimizer, scheduler)
            print("origin loss: ", origin_loss[step_id], "current loss: ", loss.numpy())
            np.testing.assert_allclose(loss.numpy(), origin_loss[step_id], rtol=1e-3)
