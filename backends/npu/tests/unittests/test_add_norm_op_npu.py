...
def add_norm(x, y, weight, bias, epsilon, begin_norm_axis):
    return _reference_layer_norm_naive(x + y, weight, bias, epsilon, begin_norm_axis)[0]


class TestNPUAddNorm(OpTest):
    def setUp(self):
        self.op_type = "add_norm"
        self.set_npu()
        self.init_dtype()

        np.random.seed(1024)
        x = np.random.uniform(-1, 1, [1, 32, 128]).astype(self.dtype)
        y = np.random.uniform(-1, 1, [1, 32, 128]).astype(self.dtype)
        ...
        out = add_norm(x, y, weight, bias, epsilon, begin_norm_axis)

        self.inputs = {"X": x, "Y": y, "Weight": weight, "Bias": bias}
