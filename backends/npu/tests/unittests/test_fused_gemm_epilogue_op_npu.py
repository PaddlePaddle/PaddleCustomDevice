...
def gelu(x):
    y_ref = (
        0.5 * x * (1.0 + np.tanh(np.sqrt(2 / np.pi) * (x + 0.044715 * np.power(x, 3))))
    )
...
class TestFuseGemmBase(OpTest):
    def setUp(self):
        ...
        self.inputs = {
            ...
            "Bias": np.random.random(self.bias_shape).astype(self.dtype) - 0.5,
        }
...
class TestEagerFusedGemmEpilogue(unittest.TestCase):
    ...
    def test_case_act(self):
        ...
        out_grad_np1 = np.random.randint(low=-20, high=20, size=out_np1.shape).astype(
            np.float32
        )
