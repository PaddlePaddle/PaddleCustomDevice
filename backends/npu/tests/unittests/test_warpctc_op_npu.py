...
class CTCForward(object):
    def __init__(
        ...
        self.loss = np.zeros([self.batch_size], dtype=softmax.dtype)
...
class TestWarpCTCOp(OpTest):
    ...
    def setUp(self):
        ...
        new_labels = np.full(
            [self.batch_size, max_target_seq_length], self.blank, dtype="int32"
        )

        cur = 0
        for batch_id in range(self.batch_size):
            for i in range(self.labels_length[batch_id]):
                new_labels[batch_id, i] = labels[cur + i, 0]
