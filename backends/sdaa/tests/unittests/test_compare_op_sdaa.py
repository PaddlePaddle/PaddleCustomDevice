...
def create_bf16_case(op_type, callback):
    class TestCompareOpBF16Op(OpTest):
        def setUp(self):
            self.set_sdaa()
            self.place = paddle.CustomPlace("sdaa", 0)
            self.op_type = op_type
        ...
        def test_output(self):
            paddle.enable_static()
            x = np.random.random(size=(1, 1, 2, 2)).astype(np.float32)
            y = np.random.random(size=(1, 1, 2, 2)).astype(np.float32)
            out = callback(x, y)
            self.inputs = {
        ...
        def test_broadcast_output(self):
            paddle.enable_static()
            x = np.arange(1, 7).reshape((1, 2, 1, 3)).astype(np.float32)
            y = np.arange(0, 6).reshape((1, 2, 3, 1)).astype(np.float32)
            out = callback(x, y)
            self.inputs = {
                "X": convert_float_to_uint16(x),
                "Y": convert_float_to_uint16(y),
