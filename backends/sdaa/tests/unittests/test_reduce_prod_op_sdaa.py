#   Copyright (c) 2022 PaddlePaddle Authors. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from __future__ import print_function

import unittest
import numpy as np
from op_test import OpTest, skip_check_grad_ci
import paddle
import paddle.base.core as core

paddle.enable_static()


def raw_reduce_prod(x, dim=[0], keep_dim=False):
    return paddle.prod(x, dim, keep_dim)


class TestReduceProd(OpTest):
    def setUp(self):
        self.op_type = "reduce_prod"
        self.python_api = raw_reduce_prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {"X": np.random.random((5, 6, 10)).astype(self.dtype)}
        self.attrs = {"dim": [0]}
        self.outputs = {"Out": self.inputs["X"].prod(axis=tuple(self.attrs["dim"]))}

    def test_check_output(self):
        self.check_output_with_place(self.place)

    def set_sdaa(self):
        self.__class__.use_custom_device = True
        self.place = paddle.CustomPlace("sdaa", 0)

    def init_dtype(self):
        self.dtype = np.float32


class TestReduceProd2(TestReduceProd):
    def setUp(self):
        self.op_type = "reduce_prod"
        self.python_api = raw_reduce_prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {"X": np.random.random((5, 6, 10)).astype(self.dtype)}
        self.attrs = {}  # default 'dim': [0]
        self.outputs = {"Out": self.inputs["X"].prod(axis=tuple([0]))}


class TestReduceProd3(TestReduceProd):
    def setUp(self):
        self.op_type = "reduce_prod"
        self.python_api = raw_reduce_prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {"X": np.random.random((5, 6, 10)).astype(self.dtype)}
        # self.attrs = {'dim': [0]}
        self.outputs = {"Out": self.inputs["X"].prod(axis=tuple([0]))}


class TestReduceProd6D(TestReduceProd):
    def setUp(self):
        self.op_type = "reduce_prod"
        self.python_api = raw_reduce_prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {"X": np.random.random((5, 6, 2, 3, 4, 2)).astype(self.dtype)}
        self.attrs = {"dim": [2, 3, 4]}
        self.outputs = {"Out": self.inputs["X"].prod(axis=tuple(self.attrs["dim"]))}


class TestReduceProd8D(TestReduceProd):
    def setUp(self):
        self.op_type = "reduce_prod"
        self.python_api = raw_reduce_prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {
            "X": np.random.random((2, 5, 3, 2, 2, 3, 4, 2)).astype(self.dtype)
        }
        self.attrs = {"dim": [2, 3, 4]}
        self.outputs = {"Out": self.inputs["X"].prod(axis=tuple(self.attrs["dim"]))}


class TestReduceAll(TestReduceProd):
    def setUp(self):
        self.op_type = "reduce_prod"
        # When test reduce all the default value for axis is None, so we use paddle.prod instead of raw function
        self.python_api = paddle.prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {"X": np.random.random((5, 6, 10)).astype(self.dtype)}
        self.attrs = {"reduce_all": True}
        self.outputs = {"Out": self.inputs["X"].prod()}


@skip_check_grad_ci("right now not implement grad op")
class TestReduceProdWithOutDtype_fp16(TestReduceProd):
    def setUp(self):
        self.op_type = "reduce_prod"
        self.python_api = raw_reduce_prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {"X": np.random.random((5, 6, 10)).astype(self.dtype)}
        self.attrs = {"dim": [0], "out_dtype": int(core.VarDesc.VarType.FP16)}
        self.outputs = {
            "Out": self.inputs["X"]
            .prod(axis=tuple(self.attrs["dim"]))
            .astype(np.float16)
        }

    def test_check_output(self):
        self.check_output_with_place(self.place, atol=1e-3)

    def init_dtype(self):
        self.dtype = np.float16


class TestReduceProdWithOutDtype_fp32(TestReduceProd):
    def setUp(self):
        self.op_type = "reduce_prod"
        self.python_api = raw_reduce_prod
        self.set_sdaa()
        self.init_dtype()

        self.inputs = {"X": np.random.random((5, 6, 10)).astype(self.dtype)}
        self.attrs = {"dim": [0], "out_dtype": int(core.VarDesc.VarType.FP32)}
        self.outputs = {
            "Out": self.inputs["X"]
            .prod(axis=tuple(self.attrs["dim"]))
            .astype(np.float32)
        }


if __name__ == "__main__":
    unittest.main()
