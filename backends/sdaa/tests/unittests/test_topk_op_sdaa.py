...
def get_cpu_results(input_data, dtype, k, axis=-1, largest=True, sorted=True):
    ori_device = paddle.get_device()

    paddle.set_device("cpu")
    x = paddle.to_tensor(input_data, dtype=dtype, stop_gradient=False)
    value, _ = paddle.topk(x, k=k, axis=axis, largest=largest, sorted=sorted)
    loss = paddle.mean(value)
    loss.backward()
...
class TestTopkV2SDAAOp(OpTest):
    ...
    def test_check_grad(self):
        if self.dtype is np.int32 or self.dtype is np.int64:
            return

        user_defined_grads = get_cpu_results(
        ...
        self.check_grad_with_place(
            self.place, ["X"], "Out", user_defined_grads=user_defined_grads
        )
...
class TestTopkV2OpFloat16(TestTopkV2SDAAOp):
    def set_attrs(self):
        self.k = 3
        self.axis = 1
        self.largest = True
...
class TestTopkV2OP2Int32(TestTopkV2SDAAOp):
    def set_attrs(self):
        self.k = 4
        self.axis = 0
        self.largest = False
...
class TestTopkV2OP3Int32(TestTopkV2SDAAOp):
    def set_attrs(self):
        self.k = 6
        self.axis = 1
        self.largest = True
...
class TestTopkV2OP4Int32(TestTopkV2SDAAOp):
    def set_attrs(self):
        self.k = 3
        self.axis = 1
        self.largest = True
...
class TestTopkV2OpDet(TestTopkV2SDAAOp):
    def set_attrs(self):
        self.k = 90747
        self.axis = 0
        self.largest = True
    ...
    def set_input_data(self):
        # numpy cannot generate a large of non repeating random arrays, which replace a squence
        integer_sequence = np.arange(73728001)
        self.input_data = integer_sequence[100:7372900].astype(self.dtype)
...
class TestTopKAPI(unittest.TestCase):
    ...
    def test_cases(self):
        places = [core.CustomPlace("sdaa", 0)]
        for place in places:
