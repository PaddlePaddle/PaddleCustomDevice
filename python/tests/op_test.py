...
def get_numeric_gradient(
    ...
    def __get_elem__(tensor, i):
        if tensor_to_check_dtype == np.float16:
            ...
            raise TypeError("Unsupported test data type %s." % tensor_to_check_dtype)
...
class OpTest(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        if not hasattr(cls, "no_need_check_grad") and not is_empty_grad_op(cls.op_type):
    ...
    def _append_ops(self, block):
        ...
        inputs = append_input_output(block, op_proto, self.inputs, True, self.dtype)
        ...
        for var_proto in proto_list:
            ...
            if var_proto.duplicable:
                ...
                for (name, np_value) in np_list[name]:
                    v = create_var(np_value, name, is_input, if_return_inputs_grad_dict)
                    var_list.append(v)
                    if if_return_inputs_grad_dict:
                        inputs_grad_dict[name] = v
    ...
    def _calc_python_api_output(self, place, egr_inps=None, egr_oups=None):
        ...
        def prepare_python_api_arguments(api, op_proto_ins, op_proto_attrs, kernel_sig):
    ...
    def _construct_grad_program_from_forward(
        ...
        for arg in grad_op_desc.input_arg_names() + grad_op_desc.output_arg_names():
            ...
            assert fwd_var is not None, "{} cannot be found".format(fwd_var_name)
    ...
    def check_output_with_place(
        ...
        class DygraphChecker(Checker):
            ...
            def _compare_numpy(self, name, actual_np, expect_np):
                if (
                    functools.reduce(lambda x, y: x * y, actual_np.shape, 1) == 0
                    and functools.reduce(lambda x, y: x * y, expect_np.shape, 1) == 0
                ):
                    pass
                else:
                    self.op_test.assertTrue(
        ...
        class EagerChecker(DygraphChecker):
            ...
            def convert_uint16_to_float_ifneed(self, actual_np, expect_np):
                with _test_eager_guard():
                    return super().convert_uint16_to_float_ifneed(actual_np, expect_np)
        ...
        if check_dygraph:
            ...
            for grad in dygraph_grad:
                if grad.dtype == np.uint16:
                    grad = convert_uint16_to_float(grad)
                    max_relative_error = (
                        0.03 if max_relative_error < 0.03 else max_relative_error
                    )
    ...
    def _get_dygraph_grad(
        ...
        with fluid.dygraph.base.guard(place=place):
            ...
            if self.dtype == np.uint16:
                cast_inputs = self._find_var_in_dygraph(outputs, output_names[0])
